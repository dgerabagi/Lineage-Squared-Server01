...
public class FreyaHard extends Reflection {
	...
	private class FirstStage extends RunnableImpl {
		...
		public void runImpl() throws Exception {
			...
			firstStageGuardSpawn = ThreadPoolManager.getInstance().scheduleAtFixedRate(new GuardSpawnTask(4), 2000L,
					50000L);
		}
	}
		...
		public void runImpl() {
			...
			for (Player p : getPlayers())
				p.sendPacket(new ExShowScreenMessage(NpcString.BEGIN_STAGE_2_FREYA, 6000, ScreenMessageAlign.TOP_CENTER,
						true, 1, -1, true));
			secondStageGuardSpawn = ThreadPoolManager.getInstance().scheduleAtFixedRate(new GuardSpawnTask(4), 2000L,
					60000L);
	...
	private class KnightCaptainSpawn extends RunnableImpl {
		...
		public void runImpl() {
			...
			for (NpcInstance n : getNpcs())
				n.unblock();
			NpcInstance knightLeader = addSpawnWithoutRespawn(IceKnightLeaderHard,
	...
	private class ThirdStage extends RunnableImpl {
		...
		public void runImpl() {
			...
			thirdStageGuardSpawn = ThreadPoolManager.getInstance().scheduleAtFixedRate(new GuardSpawnTask(4), 2000L,
					50000L);
			NpcInstance freyaStand = addSpawnWithoutRespawn(FreyaStandHard,
