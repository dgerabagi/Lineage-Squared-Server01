 ...
 * Содержит список границ включающих в себя территорию и исключающихся из
 * территории.
 * 
 * @author G1ta0
 */
public class Territory implements Shape, SpawnRange {
	...
	/**
	 * A helper to add a single-point polygon with z-min,z-max.
	 */
	public Territory add(int x, int y, int zmin, int zmax) {
		// Typically we use a polygon shape that has 1 or more points
		// from the l2ft.commons.geometry package.
		l2ft.commons.geometry.Polygon poly = new l2ft.commons.geometry.Polygon();
		poly.add(x, y).setZmin(zmin).setZmax(zmax);
		return add(poly);
	}
