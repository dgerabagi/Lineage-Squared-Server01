...
public final class ItemFunctions
{
	...
	public final static SystemMessage2 checkIfCanEquip(PetInstance pet, ItemInstance item)
	{
		...
		if(item.getTemplate().getItemClass() == ItemClass.CONSUMABLE)
			return null;
	...
	public final static L2GameServerPacket checkIfCanEquip(Player player, ItemInstance item)
	{
		...
		if((player.getRace() == Race.kamael || player.getActiveClass().isKamael()) && (item.getItemType() == ArmorType.HEAVY || item.getItemType() == ArmorType.MAGIC || item.getItemType() == ArmorType.SIGIL || item.getItemType() == WeaponType.NONE))
			return new SystemMessage2(SystemMsg.YOU_DO_NOT_MEET_THE_REQUIRED_CONDITION_TO_EQUIP_THAT_ITEM);

		// не камаэли и рапиры/арбалеты/древние мечи
		if((player.getRace() != Race.kamael || !player.getActiveClass().isKamael()) && (item.getItemType() == WeaponType.CROSSBOW || item.getItemType() == WeaponType.RAPIER || item.getItemType() == WeaponType.ANCIENTSWORD))
			return new SystemMessage2(SystemMsg.YOU_DO_NOT_MEET_THE_REQUIRED_CONDITION_TO_EQUIP_THAT_ITEM);
		...
		if(item.getTemplate().isCloak())
		{
			...
			// Плащи для камаэлей // Raincoats for Kamael
			if(item.getName().contains("Kamael") && player.getRace() != Race.kamael)
				return new SystemMessage2(SystemMsg.YOU_DO_NOT_MEET_THE_REQUIRED_CONDITION_TO_EQUIP_THAT_ITEM);

			// Плащи можно носить только с S80 или S84 сетом // Coats can be worn only with the S80 or S84 Seth
		...
		if(targetSlot == ItemTemplate.SLOT_DECO)
		{
			...
			for(int slot = Inventory.PAPERDOLL_DECO1; slot <= Inventory.PAPERDOLL_DECO6; slot++)
			{
				...
				{
					if(deco == item)
						return null; // талисман уже одет и количество слотов больше нуля // Mascot is wearing, and the number of slots is greater than zero
					// Проверяем на количество слотов и одинаковые талисманы // Check for the number of slots and the same mascots
