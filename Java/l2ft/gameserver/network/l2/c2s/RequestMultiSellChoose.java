						...
						for(int i = 0; i < ingridientItemCount * _amount; i++)
						{
							...
							if(keepenchant)
							{
								...
								for(ItemInstance item : list)
								{
									...
									if(!items.contains(itmd) && item.canBeExchanged(activeChar))
									{
										itemToTake = item;
										break;
									}
			...
			for(ItemData id : items)
			{
				...
				if(count > 0)
					if(id.getId() == ItemTemplate.ITEM_ID_CLAN_REPUTATION_SCORE)
					{
						...
						activeChar.sendPacket(new SystemMessage2(SystemMsg.S1_POINTS_HAVE_BEEN_DEDUCTED_FROM_THE_CLANS_REPUTATION_SCORE).addInteger(count));
					}
			...
			for(MultiSellIngredient in : entry.getProduction())
				if(in.getItemId() <= 0)
				{
					if(in.getItemId() == ItemTemplate.ITEM_ID_CLAN_REPUTATION_SCORE)
					{
						...
						activeChar.sendPacket(new SystemMessage2(SystemMsg.YOUR_CLAN_HAS_ADDED_1S_POINTS_TO_ITS_CLAN_REPUTATION_SCORE).addInteger(in.getItemCount() * _amount));
					}
