...
public class CharacterCreate extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		else if(_race == Race.kamael.ordinal())
		{
			//TODO custom msg
			return;
		}
	...
	private void initNewChar(GameClient client, Player newChar)
	{
		...
		for(SkillLearn skill : SkillAcquireHolder.getInstance().getAvailableSkills(newChar, AcquireType.NORMAL))
			newChar.addSkill(SkillTable.getInstance().getInfo(skill.getId(), skill.getLevel()), false);
		...
		// ĐżĐľĐ˝ŃŹĐ» ĐşĐ°Đş Đ˝Đ° ĐżĐ°Đ˝ĐµĐ»ŃŚĐşĐµ ĐľŃ‚ĐľĐ±Ń€Đ°Đ·Đ¸Ń‚ŃŚ. Đ˝Ń† Ń�ĐľŃ„Ń‚ 10-11 ĐżĐ°Đ˝ĐµĐ»Đ¸ Ń�Đ´ĐµĐ»Đ°Đ»Đ¸(by VISTALL)
		// fly transform
		...
		newChar.setCurrentHpMp(500, 500);
