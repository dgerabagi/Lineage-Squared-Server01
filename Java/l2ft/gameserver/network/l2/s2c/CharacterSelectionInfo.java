...
public class CharacterSelectionInfo extends L2GameServerPacket
{
	...
	protected final void writeImpl()
	{
		...
		writeC(0); //Ń€Đ°Đ·Ń€ĐµŃ�Đ°ĐµŃ‚ Đ¸Đ»Đ¸ Đ·Đ°ĐżŃ€ĐµŃ‰Đ°ĐµŃ‚ Ń�ĐľĐ·Đ´Đ°Đ˝Đ¸Đµ Đ¸ĐłŃ€ĐľĐşĐľĐ˛
		...
		for(int i = 0; i < size; i++)
		{
			...
			if(weaponId == 8190) // Transform id (Đ˝Đ° ĐľŃ„Ń„Đµ ĐľŃ‚ĐľĐ±Ń€Đ°Đ¶Đ°ŃŽŃ‚Ń�ŃŹ Ń‚ĐľĐ»ŃŚĐşĐľ ĐšĐ’ Ń‚Ń€Đ°Đ˝Ń�Ń„ĐľŃ€ĐĽĐ°Ń†Đ¸Đ¸ Đ¸Đ»Đ¸ Đ˛ĐľĐľĐ±Ń‰Đµ Đ˝Đµ ĐľŃ‚ĐľĐ±Ń€Đ°Đ¶Đ°ŃŽŃ‚Ń�ŃŹ ;)
				writeD(301);
	...
	private static CharSelectInfoPackage restoreChar(ResultSet chardata)
	{
		...
		try
		{
			...
			int primaryClass = chardata.getInt("primary_class_template");
			...
			boolean secondaryClassMage = ClassId.values()[chardata.getInt("secondClassId")].isMage();
			PlayerTemplate templ = CharTemplateTable.getInstance().getTemplate(primaryClass, baseClassId, secondaryClassMage, female);
			if(templ == null)
			{
			...
			charInfopackage.setRace(ClassId.values()[primaryClass].getRace().ordinal());
			charInfopackage.setClassId(classid);
			charInfopackage.setBaseClassId(primaryClass);
