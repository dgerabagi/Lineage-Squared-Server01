	...
	protected boolean checkAggression(Creature target)
	{
		...
		if(target.isPlayable())
		{
			...
			if(actor.isMonster() && target.isInZonePeace())
				return false;
			if(((Playable)target).isAggroProtected())
				return false;
		}
			...
			if(minionList != null && minionList.hasAliveMinions())
				for(MinionInstance minion : minionList.getAliveMinions())
				{
					if(attacker.isPlayer())
						attacker.sendMessage("evt aggression, damage:"+damage);
					minion.getAI().notifyEvent(CtrlEvent.EVT_AGGRESSION, attacker, (damage+1));
				}
