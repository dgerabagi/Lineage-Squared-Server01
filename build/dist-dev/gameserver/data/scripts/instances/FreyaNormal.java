...
import l2ft.gameserver.network.l2.s2c.ExChangeClientEffectInfo;
...
public class FreyaNormal extends Reflection {
	...
	private static final int[] _eventTriggers = { 23140202, 23140204, 23140206, 23140208, 23140212, 23140214,
			23140216 };
	...
	private static Territory centralRoom = new Territory().add(new Polygon().add(114264, -113672).add(113640, -114344)
			.add(113640, -115240).add(114264, -115912).add(115176, -115912).add(115800, -115272).add(115800, -114328)
			.add(115192, -113672).setZmax(-11225).setZmin(-11225));
	...
	private class PreStage extends RunnableImpl {
		...
		public void runImpl() throws Exception {
			...
			// screen message
			for (Player player : getPlayers())
				player.sendPacket(new ExShowScreenMessage(NpcString.BEGIN_STAGE_1_FREYA, 6000,
						ScreenMessageAlign.TOP_CENTER, true, 1, -1, true));
			// spawning few guards
			for (int i = 0; i < 10; i++)
	...
	private class FirstStage extends RunnableImpl {
		...
		public void runImpl() throws Exception {
			...
			for (Player player : getPlayers())
				player.sendPacket(new ExShowScreenMessage(NpcString.FREYA_HAS_STARTED_TO_MOVE, 4000,
						ScreenMessageAlign.MIDDLE_CENTER, true));
			...
			firstStageGuardSpawn = ThreadPoolManager.getInstance().scheduleAtFixedRate(new GuardSpawnTask(1), 2000L,
					30000L);
		}
	}
	...
	private class SecondStage extends RunnableImpl {
		...
		public void runImpl() throws Exception {
			...
			for (Player p : getPlayers())
				p.sendPacket(new ExShowScreenMessage(NpcString.BEGIN_STAGE_2_FREYA, 6000, ScreenMessageAlign.TOP_CENTER,
						true, 1, -1, true));
			secondStageGuardSpawn = ThreadPoolManager.getInstance().scheduleAtFixedRate(new GuardSpawnTask(2), 2000L,
					30000L);
	...
	private class KnightCaptainSpawn extends RunnableImpl {
		...
		public void runImpl() throws Exception {
			...
			for (NpcInstance n : getNpcs())
				n.unblock();
			NpcInstance knightLeader = addSpawnWithoutRespawn(IceKnightLeaderNormal,
					new Location(114707, -114799, -11199, 15956), 0);
	...
	private class ThirdStage extends RunnableImpl {
		...
		public void runImpl() throws Exception {
			...
			manageStorm(true);
			for (Player p : getPlayers()) {
				p.sendPacket(new ExShowScreenMessage(NpcString.BEGIN_STAGE_3_FREYA, 6000, ScreenMessageAlign.TOP_CENTER,
						true, 1, -1, true));
				p.sendPacket(new ExChangeClientEffectInfo(2));
			}
			thirdStageGuardSpawn = ThreadPoolManager.getInstance().scheduleAtFixedRate(new GuardSpawnTask(3), 2000L,
					30000L);
			NpcInstance freyaStand = addSpawnWithoutRespawn(FreyaStandNormal,
					new Location(114720, -117085, -11088, 15956), 0);
	...
	private class ForthStage extends RunnableImpl {
		...
		public void runImpl() throws Exception {
			...
			for (Player p : getPlayers()) {
				p.unblock();
				p.sendPacket(new ExShowScreenMessage(NpcString.BEGIN_STAGE_4_FREYA, 6000, ScreenMessageAlign.TOP_CENTER,
						true, 1, -1, true));
			}
	...
	// TODO: FIX Freya and Frintezza minimum to 5
