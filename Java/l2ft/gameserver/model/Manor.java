// java/l2ft/gameserver/model/Manor.java
...
import l2ft.gameserver.model.Manor.SeedData;
...
public class Manor {
	...
	public int getSeedBasicPriceByCrop(int cropId) {
		for (SeedData seed : _seeds.values())
			if (seed.getCrop() == cropId)
				return (int) getSeedBasicPrice(seed.getId());
		return 0;
	}
	...
	public synchronized long getRewardAmountPerCrop(int castle, int cropId, int type) {
		final CropProcure cs = ResidenceHolder.getInstance().getResidence(Castle.class, castle)
				.getCropProcure(CastleManorManager.PERIOD_CURRENT).get(cropId);
		for (SeedData seed : _seeds.values())
			if (seed.getCrop() == cropId)
...
// EOF Java/l2ft/gameserver/model/Manor.java
