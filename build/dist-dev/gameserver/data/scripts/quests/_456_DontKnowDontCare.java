 ...
 *			WARNING! This quest can be done not only a group, but a pipeline, all the characters in the team
 *         канале имеют шанс получить квестовые предметы. После убийства боссов будут появляться специальные НПЦ - мертвые тела боссов, 
 *			channel have a chance to get the quest items. After killing the bosses will appear special NPC - dead bodies bosses,
 *		   для получения квестовых предметов необходимо будет "поговорить" с этим НПЦ.
 *			to get the quest items will need to "talk" with the NPC.
 */
