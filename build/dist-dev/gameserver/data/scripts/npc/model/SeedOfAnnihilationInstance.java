...
public class SeedOfAnnihilationInstance extends MonsterInstance
{
	...
	private static final int FAME_FOR_MOB = 180;
	@Override
	protected void onDeath(Creature killer)
	{
		...
		if(killer.isPlayable())
		{
			...
			{
				player.setFame(player.getFame()+FAME_FOR_MOB, "mobFromFameZone");
			}
			else
			{
				int famePerMember = (int) Math.floor(FAME_FOR_MOB/player.getParty().getMemberCount());
				for(Player member : player.getParty())
					member.setFame(member.getFame()+famePerMember, "mobFromFameZone");
			}
		}
