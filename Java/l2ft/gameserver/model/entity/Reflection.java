...
import l2ft.gameserver.model.entity.Reflection.ReflectionListenerList;
...
public class Reflection {
	...
	public void minuteBeforeCollapse() {
		...
		try {
			for (GameObject o : _objects)
				if (o.isPlayer())
					((Player) o).sendPacket(new SystemMessage(
							SystemMessage.THIS_INSTANCE_ZONE_WILL_BE_TERMINATED_IN_S1_MINUTES_YOU_WILL_BE_FORCED_OUT_OF_THE_DANGEON_THEN_TIME_EXPIRES)
							.addNumber(1));
		} finally {
			lock.unlock();
		}
	...
	public void collapse() {
		...
		// Check ForgottenBattlegrounds
		ForgottenBattlegroundsEvent fbgEvent = ForgottenBattlegroundsManager.getInstance().getRunningEvent();
		if (fbgEvent != null) {
			Reflection fbgReflection = fbgEvent.getReflection();
			if (fbgReflection != null && fbgReflection.getId() == this.getId()) {
				// That means this reflection is hosting an active FGB
				try {
					if (fbgEvent.isInProgress()) {
						fbgEvent.stopEvent(); // or event.endEvent(...) etc.
					}
				} catch (Exception e) {
					_log.error("Error forcibly stopping FGB event in reflection collapse: " + e.getMessage(), e);
				}
			}
		}

		// NEW - Check IslandAssault
		IslandAssaultEvent iaEvent = IslandAssaultManager.getInstance().getRunningEvent();
		if (iaEvent != null) {
			Reflection iaReflection = iaEvent.getReflection();
			if (iaReflection != null && iaReflection.getId() == this.getId()) {
				// This reflection is hosting active Island Assault
				try {
					if (iaEvent.isInProgress())
						iaEvent.stopEvent();
				} catch (Exception e) {
					_log.error("Error forcibly stopping IslandAssault event in reflection collapse: " + e.getMessage(),
							e);
				}
			}
		}
			...
			for (Player player : teleport) {
				if (player.getParty() != null) {
					...
					if (player.getParty().getCommandChannel() != null
							&& equals(player.getParty().getCommandChannel().getReflection()))
	...
	public void clearReflection(int timeInMinutes, boolean message) {
		...
		if (message)
			for (Player pl : getPlayers())
				if (pl != null)
					pl.sendPacket(new SystemMessage(SystemMessage.THIS_DUNGEON_WILL_EXPIRE_IN_S1_MINUTES)
							.addNumber(timeInMinutes));
	}
			...
			for (int objectId : players) {
				try {
					...
					if (player != null)
						...
						mysql.set("REPLACE INTO character_instances (obj_id, id, reuse) VALUES (?,?,?)", objectId,
								getInstancedZoneId(), time);
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
