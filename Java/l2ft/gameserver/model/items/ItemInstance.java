// java/l2ft/gameserver/model/items/ItemInstance.java
...
import l2ft.gameserver.model.items.ItemInstance.FuncAttack;
import l2ft.gameserver.model.items.ItemInstance.FuncDefence;
...
public final class ItemInstance extends GameObject implements JdbcEntity {
	...
	public ItemInstance(int objectId, int itemId) {
		super(objectId);
		setItemId(itemId);
		setLifeTime(getTemplate().isTemporal()
				? (int) (System.currentTimeMillis() / 1000L) + getTemplate().getDurability() * 60
				: getTemplate().getDurability());
	...
	public void dropToTheGround(Player lastAttacker, NpcInstance fromNpc) {
		...
		if (lastAttacker != null) // lastAttacker в данном случае top damager
		{
			...
			_dropTimeOwner = System.currentTimeMillis() + Config.NONOWNER_ITEM_PICKUP_DELAY
					+ (fromNpc != null && fromNpc.isRaid() ? 285000 : 0);
		}
	...
	@Override
	public void delete() {
		// logItemDeletionEvent(); this line goes with the function to store deleted
		// items.
		_itemsDAO.delete(this);
	}

	/*
	 * Created by accident, its a function to store deleted items in the
	 * enchantment_history.
	 * private void logItemDeletionEvent() {
	 ...
	 * "INSERT INTO enchantment_history (item_object_id, item_id, item_name, owner_id, enchant_level, action, details) VALUES (?, ?, ?, ?, ?, ?, ?)"
	 * );
	 * statement.setInt(1, getObjectId());
	 * statement.setInt(2, getItemId());
	 * statement.setString(3, getTemplate().getName());
	 * statement.setInt(4, getOwnerId());
	 * statement.setInt(5, getEnchantLevel());
	 * statement.setString(6, "item_deleted"); // Changed from "item_destroyed" to
	 * "item_deleted"
	 * statement.setString(7, "Item deleted or destroyed.");
	 * statement.executeUpdate();
	 * } catch (SQLException e) {
	 * _log.error("Failed to log item deletion for item: " + this, e);
	 * } finally {
...
// EOF Java/l2ft/gameserver/model/items/ItemInstance.java
