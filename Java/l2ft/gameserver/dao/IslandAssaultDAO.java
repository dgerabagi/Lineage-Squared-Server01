...
public class IslandAssaultDAO {

    private static final Logger _log = LoggerFactory.getLogger(IslandAssaultDAO.class);

    private static final IslandAssaultDAO _instance = new IslandAssaultDAO();

    public static IslandAssaultDAO getInstance() {
        return _instance;
    }

    private static final String SELECT_PLAYER_STATS = "SELECT charId, totalKills, totalDeaths, totalWins, totalLosses, flagsDestroyed, currentELO, highestELO, totalMatches, lastActive, leaves FROM iab_player_stats WHERE charId=?";
    private static final String INSERT_PLAYER_STATS = "INSERT INTO iab_player_stats (charId, currentELO, highestELO, lastActive) VALUES (?,2000,2000,?)";
    private static final String UPDATE_PLAYER_STATS = "UPDATE iab_player_stats SET totalKills=?, totalDeaths=?, totalWins=?, totalLosses=?, flagsDestroyed=?, currentELO=?, highestELO=?, totalMatches=?, lastActive=? WHERE charId=?";

    private static final String INSERT_MATCH = "INSERT INTO iab_matches (startTime,winnerTeam,blueTeamSize,redTeamSize) VALUES(?,?,?,?)";
    private static final String GET_LAST_INSERT_ID = "SELECT LAST_INSERT_ID()";
    private static final String UPDATE_MATCH = "UPDATE iab_matches SET endTime=?, winnerTeam=?, blueFlagsDestroyed=?, redFlagsDestroyed=? WHERE matchId=?";

    private static final String INSERT_MATCH_PARTICIPANT = "INSERT INTO iab_match_participants (matchId,charId,team,kills,deaths,oldELO,primaryClassId,secondaryClassId) VALUES(?,?,?,?,?,?,?,?)";
    private static final String UPDATE_MATCH_PARTICIPANT = "UPDATE iab_match_participants SET kills=?, deaths=?, newELO=? WHERE matchId=? AND charId=?";
    private static final String UPDATE_MATCH_PARTICIPANT_LEFT_EARLY = "UPDATE iab_match_participants SET leftEarly=1 WHERE matchId=? AND charId=?";

    private static final String INSERT_ELO_HISTORY = "INSERT INTO iab_elo_history (charId,timestamp,oldELO,newELO,matchId) VALUES(?,?,?,?,?)";

    private static final String UPDATE_LIVE_STATUS = "UPDATE iab_live_status SET eventLive=?, currentMatchId=?, blueFlagsRemaining=?, redFlagsRemaining=?, blueFlagsDestroyed=?, redFlagsDestroyed=?, blueTeamSize=?, redTeamSize=?, blueTotalKills=?, redTotalKills=?, lastUpdate=? WHERE id=1";

    private static final String INSERT_KILL = "INSERT INTO iab_kills (matchId, killerCharId, victimCharId, timestamp, eloDelta) VALUES (?,?,?,?,?)";
    private static final String CLEAR_LIVE_PLAYERS = "DELETE FROM iab_live_players WHERE matchId=?";
    private static final String INSERT_LIVE_PLAYER = "INSERT INTO iab_live_players (matchId,charId,team,currentELO,kills,deaths,primaryClassId,secondaryClassId,flagsDestroyed,lastUpdate) VALUES(?,?,?,?,?,?,?,?,?,?)";
    private static final String UPDATE_LIVE_PLAYER = "UPDATE iab_live_players SET currentELO=?, kills=?, deaths=?, flagsDestroyed=?, lastUpdate=? WHERE matchId=? AND charId=?";
    private static final String INCREMENT_LEAVES = "UPDATE iab_player_stats SET leaves=leaves+1 WHERE charId=?";
    private static final String INSERT_FLAG_CAPTURE = "INSERT INTO iab_flag_captures (matchId,charId,timestamp,flagIndex) VALUES(?,?,?,?)";
