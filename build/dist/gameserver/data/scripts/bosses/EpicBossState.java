...
public class EpicBossState {
	...
	public void update() {
		...
		try {
			...
			statement.executeUpdate("UPDATE epic_boss_spawn SET respawnDate=" + _respawnDate / 1000 + ", state="
					+ _state.ordinal() + " WHERE bossId=" + _bossId);
			final Date dt = new Date(_respawnDate);
			_log.info("update EpicBossState: ID:" + _bossId + ", RespawnDate:" + dt + ", State:" + _state.toString());
		} catch (Exception e) {
			_log.error("Exception on update EpicBossState: ID " + _bossId + ", RespawnDate:" + _respawnDate / 1000
