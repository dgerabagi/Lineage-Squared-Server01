...
/**
 * ĐšĐ˛ĐµŃ�Ń‚ Đ˝Đ° Đ˛Ń‚ĐľŃ€Ń�ŃŽ ĐżŃ€ĐľŃ„ĐµŃ�Ń�Đ¸ŃŽ Test Of Witchcraft
 *
...
public class _229_TestOfWitchcraft extends Quest implements ScriptFile
{
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == Orim)
		{
			...
			{
				if(st.getPlayer().getActiveClassClassId().getId() == 0x0b || st.getPlayer().getActiveClassClassId().getId() == 0x04 || st.getPlayer().getActiveClassClassId().getId() == 0x20)
					if(st.getPlayer().getLevel() < 39)
					{
						htmltext = "30630-02.htm";
						st.exitCurrentQuest(true);
					}
					else if(st.getPlayer().getActiveClassClassId().getId() == 0x0b)
