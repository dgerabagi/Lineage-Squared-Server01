...
public class AntharasManager extends Functions implements ScriptFile, OnDeathListener {
	...
	private static final int FWA_FIXINTERVALOFANTHARAS = Config.EPIC_ANTHARAS_RESPAWN_INTERVAL * 60 * 60000;
	private static final int FWA_FIX_ANTHARAS_RANDOM = Config.EPIC_ANTHARAS_RESPAWN_RANDOM * 60 * 60000;
		...
		public void runImpl() {
			switch (_taskId) {
				...
				case 7:
					...
					_antharas.broadcastPacket(new PlaySound(PlaySound.Type.MUSIC, "BS02_A", 1, _antharas.getObjectId(),
							_antharas.getLoc()));
	...
	private static int getRespawnInterval() {
		return (int) (Rnd.get((FWA_FIXINTERVALOFANTHARAS - FWA_FIXINTERVALOFANTHARAS),
				(FWA_FIXINTERVALOFANTHARAS + FWA_FIX_ANTHARAS_RANDOM)));
	}
	...
	public static void enterTheLair(Player ccleader) {
		...
		for (Player p : cc)
			if (p.isDead() || p.isFlying() || p.isCursedWeaponEquipped()
					|| p.getInventory().getCountOf(PORTAL_STONE) < 1 || !p.isInRange(ccleader, 500)) {
				ccleader.sendMessage(
						"Command Channel member " + p.getName() + " doesn't meet the requirements to enter the nest");
				return;
			}
