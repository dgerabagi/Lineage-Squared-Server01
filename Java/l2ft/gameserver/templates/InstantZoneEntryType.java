...
public enum InstantZoneEntryType {
	...
	PARTY {
		...
		public boolean canEnter(Player player, InstantZone instancedZone) {
			...
			if (party.getMemberCount() < instancedZone.getMinParty()) {
				player.sendPacket(
						new SystemMessage2(SystemMsg.YOU_MUST_HAVE_A_MINIMUM_OF_S1_PEOPLE_TO_ENTER_THIS_INSTANT_ZONE)
								.addInteger(instancedZone.getMinParty()));
				return false;
			}
			...
			for (Player member : party.getPartyMembers()) {
				if (!player.isInRange(member, 500)) {
					party.broadCast(
							new SystemMessage2(
									SystemMsg.C1_IS_IN_A_LOCATION_WHICH_CANNOT_BE_ENTERED_THEREFORE_IT_CANNOT_BE_PROCESSED)
									.addName(member));
					return false;
				}
	...
	COMMAND_CHANNEL {
		@Override
		public boolean canEnter(Player player, InstantZone instancedZone) {
			// [CHANGED] Remove the strict requirement of having a command channel.
			// Instead, we allow a single party or a CC. Check total count vs. min/max.

			Party party = player.getParty();
			if (party == null) {
				// Must be in a party at least
				player.sendPacket(
						SystemMsg.YOU_CANNOT_ENTER_BECAUSE_YOU_ARE_NOT_ASSOCIATED_WITH_THE_CURRENT_COMMAND_CHANNEL);
				return false;
			}
			if (!party.isLeader(player)) {
				// Must be the party/CC leader
				player.sendPacket(SystemMsg.ONLY_A_PARTY_LEADER_CAN_MAKE_THE_REQUEST_TO_ENTER);
				return false;
			}

			// If a CC is present, use its total count; else use the single party's count
			CommandChannel cc = party.getCommandChannel();
			int memberCount = cc != null ? cc.getMemberCount() : party.getMemberCount();

			if (memberCount < instancedZone.getMinParty()) {
				player.sendPacket(
						new SystemMessage2(SystemMsg.YOU_MUST_HAVE_A_MINIMUM_OF_S1_PEOPLE_TO_ENTER_THIS_INSTANT_ZONE)
								.addInteger(instancedZone.getMinParty()));
				return false;
			}
			if (memberCount > instancedZone.getMaxParty()) {
				player.sendPacket(SystemMsg.YOU_CANNOT_ENTER_DUE_TO_THE_PARTY_HAVING_EXCEEDED_THE_LIMIT);
				return false;
			}

			// Check distance + personal conditions
			// We'll iterate over whichever group is relevant
			if (cc != null) {
		...
		@Override
		public boolean canReEnter(Player player, InstantZone instanceZone) {
			// [CHANGED] Similarly remove strict CC requirement, just check
			// min/max/conditions
			Party party = player.getParty();
			if (party == null) {
				player.sendPacket(
			...
			if (memberCount > instanceZone.getMaxParty()) {
				player.sendPacket(SystemMsg.YOU_CANNOT_ENTER_DUE_TO_THE_PARTY_HAVING_EXCEEDED_THE_LIMIT);
				return false;
			}
			if (player.isCursedWeaponEquipped() || player.isInFlyingTransform()) {
	...
	private static SystemMsg checkPlayer(Player player, InstantZone instancedZone) {
		...
		if (instancedZone.getRemovedItemId() > 0
				&& instancedZone.getRemovedItemNecessity()
