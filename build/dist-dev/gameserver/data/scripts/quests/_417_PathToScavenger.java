...
public class _417_PathToScavenger extends Quest implements ScriptFile
{
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		{
			st.set("id", "0");
			if(st.getPlayer().getLevel() >= 18 && st.getPlayer().getActiveClassClassId().getId() == 0x35 && st.getQuestItemsCount(RING_OF_RAVEN) == 0)
			{
				st.setCond(1);
				st.setState(STARTED);
			...
			else if(st.getPlayer().getActiveClassClassId().getId() != 0x35)
			{
				if(st.getPlayer().getActiveClassClassId().getId() == 0x36)
					htmltext = "collector_pipi_q0417_02a.htm";
				else
					htmltext = "collector_pipi_q0417_08.htm";
			}
			else if(st.getPlayer().getLevel() < 18 && st.getPlayer().getActiveClassClassId().getId() == 0x35)
				htmltext = "collector_pipi_q0417_02.htm";
			else if(st.getPlayer().getLevel() >= 18 && st.getPlayer().getActiveClassClassId().getId() == 0x35 && st.getQuestItemsCount(RING_OF_RAVEN) == 1)
