...
public class RequestAnswerJoinParty extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		if(activeChar.getActiveClassClassId().getLevel()<2 || requestor.getActiveClassClassId().getLevel()<2)
		{
			request.cancel();
			requestor.sendPacket(JoinParty.FAIL);
			return;
		...
		if(TournamentManager.getInstance().isPlayerAtTournamentStart(requestor))
		{
			request.cancel();
			activeChar.sendMessage("Party Leader is currently at tournament!");
			requestor.sendPacket(JoinParty.FAIL);
			return;
		}
