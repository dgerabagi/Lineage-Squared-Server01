...
import l2ft.gameserver.model.Creature.MoveNextTask;
...
public abstract class Creature extends GameObject {
				...
				if (!isFlying() && !isInBoat() && !isInWater() && !isBoat())
					if (loc.z - getZ() > 256) {
						String bug_text = "geo bug 1 at: " + getLoc() + " => " + loc.x + "," + loc.y + "," + loc.z
								+ "\tAll path: " + _moveList.get(0) + " => " + _moveList.get(_moveList.size() - 1);
	 ...
	 * пути
	 * при followToCharacter используется для хранения мировых координат в которых
	 * находилась последний раз преследуемая цель для отслеживания необходимости
	 * перестраивания пути
	 */
	...
	/**
	 * Список игроков, которым необходимо отсылать информацию об изменении состояния
	 * персонажа
	 */
	...
	public final void abortCast(boolean force, boolean message) {
		if (isCastingNow() && (force || canAbortCast())) {
			...
			finishFly(); // Броадкаст пакета FlyToLoc уже выполнен, устанавливаем координаты чтобы не
							// было визуальных глюков
			clearCastVars();
	...
	public boolean absorbAndReflect(Creature target, Skill skill, double damage) {
		...
		if (value > 0 && Rnd.chance(value)) {
			reduceCurrentHp(Math.min(damage, target.getMaxHp() / 2), target, null, true, true, false, false, false,
					false, true);
			return true;
		}
		...
		if (value > 0) {
			...
			if (target.getCurrentHp() + target.getCurrentCp() > damage)
				reduceCurrentHp(value / 100. * Math.min(damage, target.getMaxHp() / 2), target, null, true, true, false,
		...
		if (absorb > 0 && !target.isDamageBlocked()) {
			...
			if (getCurrentHp() < limit)
				setCurrentHp(Math.min(_currentHp + damage * absorb * Config.ALT_ABSORB_DAMAGE_MODIFIER / 100., limit),
						false);
		}
	...
	public double absorbToEffector(Creature attacker, double damage) {
		...
		if (transferToEffectorDam > 0) {
			...
			effector.reduceCurrentHp(transferDamage, effector, null, false, false, !attacker.isPlayable(), false, true,
					false, true);
		}
		return damage;
	}
	...
	public double absorbToSummon(Creature attacker, double damage) {
		...
		if (transferToSummonDam > 0) {
			...
			if (summon == null || summon.isDead() || summon.getCurrentHp() < transferDamage)
				...
				sendMessage(Math.round(transferDamage) + " damage has been transfered to Pet.");
	...
	public void broadcastStatusUpdate() {
		...
		StatusUpdate su = makeStatusUpdate(StatusUpdate.MAX_HP, StatusUpdate.MAX_MP, StatusUpdate.CUR_HP,
				StatusUpdate.CUR_MP);
					...
					if (Rnd.chance(skill.getCancelTarget()))
						if ((target.getCastingSkill() == null
								|| !(target.getCastingSkill().getSkillType() == SkillType.TAKECASTLE
										...
										|| target.getCastingSkill().getSkillType() == SkillType.TAKEFLAG))
								&& !target.isRaid()) {
	...
	public void useTriggerSkill(GameObject target, List<Creature> targets, TriggerInfo trigger, Skill owner,
		...
		// DS: Для шансовых скиллов с TARGET_SELF и условием "пвп" сам кастер будет
		// являться aimTarget,
		...
		if (Rnd.chance(trigger.getChance()) && trigger.checkCondition(this, realTarget, aimTarget, owner, damage)
			...
			List<Creature> allTargets = targets == null ? skill.getTargets(this, aimTarget, false) : targets;
	...
	public boolean checkReflectSkill(Creature attacker, Skill skill) {
		...
		if (skill.isMagic() && skill.getSkillType() != SkillType.MDAM)
			return false;
		if (Rnd.chance(calcStat(skill.isMagic() ? Stats.REFLECT_MAGIC_SKILL : Stats.REFLECT_PHYSIC_SKILL, 0, attacker,
				skill))) {
	...
	public void doCounterAttack(Skill skill, Creature attacker, boolean blow) {
		if (isDead()) // если персонаж уже мертв, контратаки быть не должно
			return;
		if (isDamageBlocked() || attacker.isDamageBlocked()) // Не контратакуем, если есть неуязвимость, иначе она может
																// отмениться
			return;
		...
		if (Rnd.chance(calcStat(Stats.COUNTER_ATTACK, 0, attacker, skill))) {
			...
			if (blow) // урон х2 для отражения blow скиллов
			{
				...
				sendPacket(new SystemMessage2(SystemMsg.C1_HAS_GIVEN_C2_DAMAGE_OF_S3).addName(this).addName(attacker)
						.addInteger((long) damage));
	...
	private void doAttackHitSimple(Attack attack, Creature target, double multiplier, boolean unchargeSS, int sAtk,
		...
		ThreadPoolManager.getInstance().schedule(
				new HitTask(this, target, damage1, crit1, miss1, attack._soulshot, shld1, unchargeSS, notify), sAtk);
	...
	private void doAttackHitByBow(Attack attack, Creature target, int sAtk) {
		...
		ThreadPoolManager.getInstance()
				.schedule(new HitTask(this, target, damage1, crit1, miss1, attack._soulshot, shld1, true, true), sAtk);
	...
	private void doAttackHitByDual(Attack attack, Creature target, int sAtk) {
		...
		// higher delay
		ThreadPoolManager.getInstance().schedule(
				new HitTask(this, target, damage1, crit1, miss1, attack._soulshot, shld1, true, false), sAtk / 2);
		ThreadPoolManager.getInstance()
				.schedule(new HitTask(this, target, damage2, crit2, miss2, attack._soulshot, shld2, false, true), sAtk);
	...
	public void doCast(Skill skill, Creature target, boolean forceUse) {
		...
		// Create a task MagicUseTask with Medium priority to launch the MagicSkill at
	...
	private Location getFlyLocation(GameObject target, Skill skill) {
		...
		if (isFlying())
			return GeoEngine.moveCheckInAir(getX(), getY(), getZ(), getX() + x1, getY() + y1, getZ(), getColRadius(),
					getGeoIndex());
	...
	protected void onDeath(Creature killer) {
		...
		if (isBlessedByNoblesse() || isSalvation()) {
			if (isSalvation() && isPlayer() && !getPlayer().isInOlympiadMode()
					&& getPlayer().getTournamentMatch() == null)
	...
	public double getMagicCriticalRate(Creature target, Skill skill) {
		return calcStat(Stats.MCRITICAL_RATE, target, skill) * 2 / 5;
	}
	...
	public Location getIntersectionPoint(Creature target) {
		...
		double range = target.getMoveSpeed() / 2; // расстояние, пройденное за 1 секунду, равно скорости. Берем
													// половину.
	...
	private boolean buildPathTo(int x, int y, int z, int offset, Creature follow, boolean forestalling,
			boolean pathFind) {
	...
	private void moveNext(boolean firstMove) {
		...
		destination = end;
		double distance = (isFlying() || isInWater()) ? begin.distance3D(end) : begin.distance(end); // клиент при
																										// передвижении
																										// не учитывает
																										// поверхность
		...
		try {
			...
			for (int i = 0; i < _zones.size(); i++) {
				...
				if (zone.getRestartPoints() != null) {
					...
					if (type == ZoneType.battle_zone || type == ZoneType.peace_zone || type == ZoneType.offshore
							|| type == ZoneType.dummy)
	...
	public void onHitTimer(Creature target, int damage, boolean crit, boolean miss, boolean soulshot, boolean shld,
			boolean unchargeSS) {
		if (isAlikeDead()) {
			sendActionFailed();
			return;
		...
		// if hitted by a cursed weapon, Cp is reduced to 0, if a cursed weapon is
		// hitted by a Hero, Cp is reduced to 0
		...
		// long nextActionTime = (_attackEndTime-System.currentTimeMillis()+10);
		// WeaponTemplate weaponItem = getActiveWeaponItem();
		// if(weaponItem == null || weaponItem.getAttackReuseDelay() <= 0 &&
		// isPlayable())
		// {
		// ThreadPoolManager.getInstance().schedule(new NotifyAITask(this,
		// CtrlEvent.EVT_READY_TO_ACT, null, null),nextActionTime);
		// }
		...
		if (checkPvP) {
			...
			if (event != null && target.isPlayable()) {
				...
				// NEW - ISLAND ASSAULT check
				IslandAssaultEvent iaEvent = getEvent(IslandAssaultEvent.class);
				if (iaEvent != null && target.isPlayable()) {
					Player atkPlayer = getPlayer();
					Player tgtPlayer = target.getPlayer();
					if (atkPlayer != null && tgtPlayer != null) {
						TeamType atkTeam = iaEvent.getTeamOfPlayer(atkPlayer);
						TeamType tgtTeam = iaEvent.getTeamOfPlayer(tgtPlayer);
						if (atkTeam != TeamType.NONE && tgtTeam != TeamType.NONE && atkTeam != tgtTeam) {
							// Opposite teams => DO NOT pvp-flag
						} else {
							startPvPFlag(target);
					...
					// Not in BG or IA => normal flag logic
					startPvPFlag(target);
				}
			}
		}
	...
	public Skill removeSkillById(Integer id) {
		...
		if (oldSkill != null) {
			removeTriggers(oldSkill);
			removeStatsOwner(oldSkill);/**
										 * if(Config.ALT_DELETE_SA_BUFFS && (oldSkill.isItemSkill() ||
										 * oldSkill.isHandler()))
										 * {
	...
	public void startSpecialEffect(int mask) {
		_abnormalEffects2 |= mask;
		sendChanges();
	}
	...
	protected void startAttackStanceTask0() {
		...
		_stanceTask = LazyPrecisionTaskManager.getInstance().scheduleAtFixedRate(
				_stanceTaskRunnable == null ? _stanceTaskRunnable = new AttackStanceTask() : _stanceTaskRunnable, 1000L,
				1000L);
	}
	...
	public void stopSpecialEffect(int mask) {
		_abnormalEffects2 &= ~mask;
	}
	...
	public void teleToLocation(int x, int y, int z, Reflection r) {
		...
		if (isPlayer()) {
			...
			// Нужно при телепорте с более высокой точки на более низкую, иначе наносится
			// вред от "падения"
	...
	public void playerTargettedMe(Player player) {
	}

	/**
	 * Выставить предельные значения HP/MP/CP и запустить регенерацию, если в этом
	 * есть необходимость
	 */
