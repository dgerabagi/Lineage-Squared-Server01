// java/l2ft/gameserver/taskmanager/DelayedItemsManager.java
			...
			if (last_payment_id_temp != last_payment_id)
				synchronized (_lock) {
					st = con.prepareStatement(
							"SELECT DISTINCT owner_id FROM items_delayed WHERE payment_status=0 AND payment_id > ?");
				...
				while (rset.next()) {
					...
					for (int i = 0; i < (stackable ? 1 : ITEM_COUNT); i++) {
						...
						if (notify && ITEM_COUNT > 0)
							player.sendPacket(
									SystemMessage2.obtainItems(ITEM_ID, stackable ? ITEM_COUNT : 1, ITEM_ENCHANT));
					}
					if (!success)
						continue;

					Log.add("<add owner_id=" + player_id + " item_id=" + ITEM_ID + " count=" + ITEM_COUNT
							+ " enchant_level=" + ITEM_ENCHANT + " payment_id=" + PAYMENT_ID + "/>", "delayed_add");
...
// EOF Java/l2ft/gameserver/taskmanager/DelayedItemsManager.java
