...
public class Formulas
{
	...
	public static AttackInfo calcPhysDam(Creature attacker, Creature target, Skill skill, boolean dual, boolean blow, boolean ss, boolean onCrit)
	{
		...
		if(isPvP)
		{
			if(skill == null)
			{
				info.damage *= attacker.calcStat(Stats.PVP_PHYS_DMG_BONUS, Config.PVP_PHYS_DMG_MULT, null, null);
				info.damage /= target.calcStat(Stats.PVP_PHYS_DEFENCE_BONUS, 1, null, null);
			}
			else
			{
				info.damage *= attacker.calcStat(Stats.PVP_PHYS_SKILL_DMG_BONUS, Config.PVP_PHYS_SKLL_DMG_MULT, null, null);
	...
	public static double calcMagicDam(Creature attacker, Creature target, Skill skill, int sps)
	{
		...
		if(isPvP && damage > 1)
		{
			damage *= attacker.calcStat(Stats.PVP_MAGIC_SKILL_DMG_BONUS, Config.PVP_MAGIC_SKILL_DMG_MULT, null, null);
		...
		if(Rnd.chance(failChance))
		{
			...
			{
				damage = 0;
				SystemMessage2 msg = new SystemMessage2(SystemMsg.C1_RESISTED_C2S_MAGIC).addName(target).addName(attacker);
			...
			{
				damage /= 2;
				SystemMessage2 msg = new SystemMessage2(SystemMsg.DAMAGE_IS_DECREASED_BECAUSE_C1_RESISTED_AGAINST_C2S_MAGIC).addName(target).addName(attacker);
				attacker.sendPacket(msg);
				target.sendPacket(msg);
			}
		}
	...
	public static boolean calcSkillSuccess(Env env, EffectTemplate et, int spiritshot)
	{
		...
		if (Config.ALT_DEBUG_ENABLED)
		{
			...
			final boolean debugPvP = Config.ALT_DEBUG_PVP_ENABLED && (debugCaster || debugTarget);
	...
	private static double getElementMod(double defense, double attack)
	{
		...
		if(diff <= 0)
			...
			return 1.2+(diff-50)*0.002;
		else if(diff < 300)
		{
			return 1.4+(diff-150)*0.002;
		}
		else if(diff<390)
			return 1.7;
		else
