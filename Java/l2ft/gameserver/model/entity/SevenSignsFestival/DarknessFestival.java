	...
	public void collapse()
	{
		...
		if(SevenSigns.getInstance().getCurrentPeriod() == SevenSigns.PERIOD_COMPETITION && getParty() != null)
		{
			...
			if(player.getInventory().destroyItem(bloodOfferings))
			{
				...
				sendCustomMessageToParticipants("l2ft.gameserver.model.entity.SevenSignsFestival.Ended");
				if(isHighestScore)
					sendMessageToParticipants("Your score is highest!");
			}
			else
				player.sendMessage(new CustomMessage("l2ft.gameserver.model.instances.L2FestivalGuideInstance.BloodOfferings", player));
		}
