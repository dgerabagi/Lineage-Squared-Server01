...
public class RegionCommunity implements ScriptFile, ICommunityBoardHandler {
	...
	private static final int[][] _towns = new int[][] { { 1010005, 19, 21 }, { 1010006, 20, 22 }, { 1010007, 22, 22 },
			{ 1010013, 22, 19 }, { 1010023, 24, 18 }, { 1010049, 23, 24 }, { 1010199, 24, 16 }, { 1010200, 21, 16 },
			{ 1010574, 22, 13 } };
	private static final String[] _regionTypes = { "&$596;", "&$597;", "&$665;" };
	private static final String[] _elements = { "&$1622;", "&$1623;", "&$1624;", "&$1625;", "&$1626;", "&$1627;" };
	private static final String[] _grade = { "&$1291;", "&$1292;", "&$1293;", "&$1294;", "&$1295;", "S80 Grade",
			"S84 Grade" };
	...
	public String[] getBypassCommands() {
		return new String[] { "_bbsloc", "_bbsregion_", "_bbsreglist_", "_bbsregsearch", "_bbsregview_",
				"_bbsregtarget_" };
	}
		...
		if ("bbsloc".equals(cmd)) {
				...
				for (Player seller : GameObjectsStorage.getAllPlayersForIterate()) {
					...
					if (tx >= rx - offset && tx <= rx + offset && ty >= ry - offset && ty <= ry + offset)
						if (seller.getPrivateStoreType() > 0
								&& seller.getPrivateStoreType() != Player.STORE_OBSERVING_GAMES)
							sellers++;
				}
				reg = reg.replace("%sellers_count%", String.valueOf(sellers));
				rl.append(reg);
			}
			HashMap<Integer, String> tpls = Util.parseTemplate(
					HtmCache.getInstance().getNotNull(Config.BBS_HOME_DIR + "bbs_region_list.htm", player));
			...
			for (int type = 0; type < _regionTypes.length; type++) {
				...
				for (Player seller : GameObjectsStorage.getAllPlayersForIterate()) {
					...
					if (tx >= rx - offset && tx <= rx + offset && ty >= ry - offset && ty <= ry + offset) {
						if (type == 0 && (seller.getPrivateStoreType() == Player.STORE_PRIVATE_SELL
								|| seller.getPrivateStoreType() == Player.STORE_PRIVATE_SELL_PACKAGE))
							sellers++;
			...
			String html = tpls.get(0);
			html = html.replace("%REGION_LIST%", rl.toString());
			html = html.replace("<?tree_menu?>",
					tpls.get(2).replace("%TREE%", "&nbsp;>&nbsp;" + HtmlUtils.htmlNpcString(town[0])));
			html = BbsUtil.htmlBuff(html, player);
			ShowBoard.separateAndSend(html, player);
			...
			if (page == 1) {
				...
				html = html.replace("%ACTION_GO_LEFT%",
						"bypass _bbsreglist_" + townId + "_" + type + "_" + (page - 1) + "_" + byItem + "_" + search);
				html = html.replace("%NPAGE%", String.valueOf(page));
				StringBuilder goList = new StringBuilder("");
				for (int i = page > 10 ? page - 10 : 1; i < page; i++)
					goList.append("<td><a action=\"bypass _bbsreglist_").append(townId).append("_").append(type)
							.append("_").append(i).append("_").append(byItem).append("_").append(search).append("\"> ")
			...
			for (int i = start; i < end; i++) {
				...
				String stpl = tpl;
				stpl = stpl.replace("%view_bypass%", "bypass _bbsregview_" + townId + "_" + type + "_" + page + "_"
						+ seller.getObjectId() + "_" + byItem + "_" + search);
				stpl = stpl.replace("%seller_name%", seller.getName());
				String title = "-";
				if (type == 0)
					title = tl != null && seller.getSellStoreName() != null && !seller.getSellStoreName().isEmpty()
							? seller.getSellStoreName()
							: "-";
				else if (type == 1)
					title = tl != null && seller.getBuyStoreName() != null && !seller.getBuyStoreName().isEmpty()
							? seller.getBuyStoreName()
							: "-";
				else if (type == 2 && seller.getPrivateStoreType() == Player.STORE_PRIVATE_MANUFACTURE)
					title = cl != null && seller.getManufactureName() != null && !seller.getManufactureName().isEmpty()
							? seller.getManufactureName()
							: "-";
			...
			html = html.replace("%TREE%", "&nbsp;>&nbsp;<a action=\"bypass _bbsregion_" + townId + "\">"
					+ HtmlUtils.htmlNpcString(town[0]) + "</a>&nbsp;>&nbsp;" + _regionTypes[type]);
			html = BbsUtil.htmlBuff(html, player);
			ShowBoard.separateAndSend(html, player);
			...
			if (seller == null || seller.getPrivateStoreType() == 0) {
				onBypassCommand(player,
						"_bbsreglist_" + townId + "_" + type + "_" + page + "_" + byItem + "_" + search);
				return;
			}
				...
				for (TradeItem ti : sl) {
					...
					if (item != null) {
						String stpl = tpl.replace("%item_name%", item.getName()
								+ (item.isEquipment() && ti.getEnchantLevel() > 0 ? " +" + ti.getEnchantLevel() : ""));
						stpl = stpl.replace("%item_img%", item.getIcon());
						stpl = stpl.replace("%item_count%", String.valueOf(ti.getCount()));
						stpl = stpl.replace("%item_price%",
								String.format("%,3d", ti.getOwnersPrice()).replace(" ", ","));

						String desc = "";
						if (item.getCrystalType() != ItemTemplate.Grade.NONE) {
							desc = _grade[item.getCrystalType().ordinal() - 1];
							desc += item.getCrystalCount() > 0
									? (player.isLangRus() ? " Кристаллов: " : " Crystals: ") + item.getCrystalCount()
											+ ";&nbsp;"
									: ";&nbsp;";
						}
						...
						if (item.isEquipment())
							if (ti.getAttackElement() >= 0 && ti.getAttackElementValue() > 0)
								desc += "&$1620;: " + _elements[ti.getAttackElement()] + " +"
										+ ti.getAttackElementValue();
							else if (ti.getDefenceFire() > 0 || ti.getDefenceWater() > 0 || ti.getDefenceWind() > 0
									|| ti.getDefenceEarth() > 0 || ti.getDefenceHoly() > 0
									|| ti.getDefenceUnholy() > 0) {
								desc += "&$1651;:";
					...
					if (item.getCrystalType() != ItemTemplate.Grade.NONE)
						desc = _grade[item.getCrystalType().ordinal() - 1]
								+ (item.getCrystalCount() > 0
										? (player.isLangRus() ? " Кристаллов: " : " Crystals: ")
												+ item.getCrystalCount() + ";&nbsp;"
										: ";&nbsp;");
			...
			html = html.replace("%list_bypass%",
					"_bbsreglist_" + townId + "_" + type + "_" + page + "_" + byItem + "_" + search);
			html = html.replace("%TREE%",
					"&nbsp;>&nbsp;<a action=\"bypass _bbsregion_" + townId + "\">" + HtmlUtils.htmlNpcString(town[0])
							+ "</a>&nbsp;>&nbsp;<a action=\"bypass _bbsreglist_" + townId + "_" + type + "_" + page
							+ "_" + byItem + "_\">" + _regionTypes[type] + "</a>&nbsp;>&nbsp;" + seller.getName());
			html = BbsUtil.htmlBuff(html, player);
			ShowBoard.separateAndSend(html, player);
	...
	private static List<Player> getSellersList(int townId, int type, String search, boolean byItem) {
		...
		for (Player seller : GameObjectsStorage.getAllPlayersForIterate()) {
			...
			if (tx >= rx - offset && tx <= rx + offset && ty >= ry - offset && ty <= ry + offset) {
				...
				List<ManufactureItem> cl = seller.getCreateList();
				if (seller.getPrivateStoreType() > 0)
					if (type == 0 && tl != null && (seller.getPrivateStoreType() == Player.STORE_PRIVATE_SELL
							|| seller.getPrivateStoreType() == Player.STORE_PRIVATE_SELL_PACKAGE))
						list.add(seller);
					else if (type == 1 && tl != null && seller.getPrivateStoreType() == Player.STORE_PRIVATE_BUY)
						list.add(seller);
					else if (type == 2 && cl != null
							&& seller.getPrivateStoreType() == Player.STORE_PRIVATE_MANUFACTURE)
						list.add(seller);
			}
		}
		...
		if (!search.isEmpty() && !list.isEmpty()) {
			...
			for (Player seller : list) {
				...
				} else if (type == 0 && tl != null && seller.getSellStoreName() != null
						&& seller.getSellStoreName().toLowerCase().contains(search))
					s_list.add(seller);
				else if (type == 1 && tl != null && seller.getBuyStoreName() != null
						&& seller.getBuyStoreName().toLowerCase().contains(search))
					s_list.add(seller);
				else if (type == 2 && cl != null && seller.getCreateList() != null
						&& seller.getManufactureName() != null
						&& seller.getManufactureName().toLowerCase().contains(search))
					s_list.add(seller);
			}
			list = s_list;
		}
