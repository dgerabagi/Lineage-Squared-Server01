...
import l2ft.gameserver.network.l2.s2c.PledgeReceiveMemberInfo;
		...
		if((activeChar.getClanPrivileges() & Clan.CP_CL_APPRENTICE) == Clan.CP_CL_APPRENTICE)
		{
			...
			if(sponsor != null && apprentice != null)
			{
				...
				if(_mode == 1)
				{
					...
					{
						activeChar.sendMessage(new CustomMessage("l2ft.gameserver.clientpackets.RequestOustAlly.MemberAlreadyHasApprentice", activeChar));
						return;
					}
					if(apprentice.hasSponsor())
					{
						activeChar.sendMessage(new CustomMessage("l2ft.gameserver.clientpackets.RequestOustAlly.ApprenticeAlreadyHasSponsor", activeChar));
						return;
					}
				...
				else
				{
					if(!sponsor.hasApprentice())
					{
						activeChar.sendMessage(new CustomMessage("l2ft.gameserver.clientpackets.RequestOustAlly.MemberHasNoApprentice", activeChar));
						return;
					}
			...
			activeChar.sendMessage(new CustomMessage("l2ft.gameserver.clientpackets.RequestOustAlly.NoMasterRights", activeChar));
	}
}