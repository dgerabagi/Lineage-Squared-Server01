// C:\l2sq\Pac Project\Java\l2ft\gameserver\model\instances\SignsPriestInstance.java
...
public class SignsPriestInstance extends NpcInstance {
	...
	public void onBypassFeedback(Player player, String command) {
		if (!canBypassCheck(player, this))
			return;
		// Remove the following lines to allow any player to interact with Mammon
			...
			switch (val) {
				...
				case 4: // Join a Cabal - SevenSigns 4 [0]1 x
					...
					if (oldCabal != SevenSigns.CABAL_NULL) {
						player.sendMessage(
								new CustomMessage("l2ft.gameserver.model.instances.L2SignsPriestInstance.AlreadyMember",
										player).addString(SevenSigns.getCabalName(cabal)));
						return;
					}
					if (player.getActiveClassClassId().level() == 0) {
						player.sendMessage(new CustomMessage(
								"l2ft.gameserver.model.instances.L2SignsPriestInstance.YouAreNewbie", player));
						break;
					}

					else if (player.getActiveClassClassId().level() >= 2)
						if (Config.ALT_GAME_REQUIRE_CASTLE_DAWN)
							if (getPlayerAllyHasCastle(player)) {
								if (cabal == SevenSigns.CABAL_DUSK) {
									player.sendMessage(new CustomMessage(
											"l2ft.gameserver.model.instances.L2SignsPriestInstance.CastleOwning",
											player));
									return;
								}
							} else
							...
							if (cabal == SevenSigns.CABAL_DAWN) {
								...
								if (Functions.getItemCount(player, SevenSigns.CERTIFICATE_OF_APPROVAL_ID) > 0) {
									...
									allowJoinDawn = true;
								} else if (Config.ALT_GAME_ALLOW_ADENA_DAWN
										&& player.getAdena() >= SevenSigns.ADENA_JOIN_DAWN_COST) {
								...
								if (!allowJoinDawn) {
									if (Config.ALT_GAME_ALLOW_ADENA_DAWN)
										player.sendMessage(new CustomMessage(
												"l2ft.gameserver.model.instances.L2SignsPriestInstance.CastleOwningCertificate",
												player));
									else
										player.sendMessage(new CustomMessage(
												"l2ft.gameserver.model.instances.L2SignsPriestInstance.CastleOwningCertificate2",
												player));
									return;
								}
							}
					...
					if (cabal == SevenSigns.CABAL_DAWN)
						player.sendPacket(Msg.YOU_WILL_PARTICIPATE_IN_THE_SEVEN_SIGNS_AS_A_MEMBER_OF_THE_LORDS_OF_DAWN); // Joined
																															// Dawn
					else
						player.sendPacket(
								Msg.YOU_WILL_PARTICIPATE_IN_THE_SEVEN_SIGNS_AS_A_MEMBER_OF_THE_REVOLUTIONARIES_OF_DUSK); // Joined
																															// Dusk
					...
					switch (newSeal) {
						case SevenSigns.SEAL_AVARICE:
							player.sendPacket(
									Msg.YOUVE_CHOSEN_TO_FIGHT_FOR_THE_SEAL_OF_AVARICE_DURING_THIS_QUEST_EVENT_PERIOD);
							break;
						case SevenSigns.SEAL_GNOSIS:
							player.sendPacket(
									Msg.YOUVE_CHOSEN_TO_FIGHT_FOR_THE_SEAL_OF_GNOSIS_DURING_THIS_QUEST_EVENT_PERIOD);
							break;
						case SevenSigns.SEAL_STRIFE:
							player.sendPacket(
									Msg.YOUVE_CHOSEN_TO_FIGHT_FOR_THE_SEAL_OF_STRIFE_DURING_THIS_QUEST_EVENT_PERIOD);
							break;
					}
				...
				case 6: // Contribute Seal Stones - SevenSigns 6 x
					...
					if (contribScore == SevenSigns.MAXIMUM_PLAYER_CONTRIB)
						...
						switch (stoneType) {
							case 1:
								blueContribCount = (SevenSigns.MAXIMUM_PLAYER_CONTRIB - contribScore)
										/ SevenSigns.BLUE_CONTRIB_POINTS;
								if (blueContribCount > blueStoneCount)
									blueContribCount = blueStoneCount;
								break;
							case 2:
								greenContribCount = (SevenSigns.MAXIMUM_PLAYER_CONTRIB - contribScore)
										/ SevenSigns.GREEN_CONTRIB_POINTS;
								if (greenContribCount > greenStoneCount)
									greenContribCount = greenStoneCount;
								break;
							case 3:
								redContribCount = (SevenSigns.MAXIMUM_PLAYER_CONTRIB - contribScore)
							...
							case 4:
								...
								redContribCount = (SevenSigns.MAXIMUM_PLAYER_CONTRIB - tempContribScore)
										/ SevenSigns.RED_CONTRIB_POINTS;
								if (redContribCount > redStoneCount)
									redContribCount = redStoneCount;
						...
						if (redContribCount > 0)
							if (player.getInventory().destroyItemByItemId(SevenSigns.SEAL_STONE_RED_ID,
									redContribCount))
								stonesFound = true;
						if (greenContribCount > 0)
							if (player.getInventory().destroyItemByItemId(SevenSigns.SEAL_STONE_GREEN_ID,
									greenContribCount))
								stonesFound = true;
						if (blueContribCount > 0) {
							ItemInstance temp = player.getInventory().getItemByItemId(SevenSigns.SEAL_STONE_BLUE_ID);
							if (player.getInventory().destroyItemByItemId(SevenSigns.SEAL_STONE_BLUE_ID,
									blueContribCount))
								stonesFound = true;
						}
						...
						if (!stonesFound) {
							player.sendMessage(new CustomMessage(
									"l2ft.gameserver.model.instances.L2SignsPriestInstance.DontHaveAnySSType", player));
							return;
						}

						contribScore = SevenSigns.getInstance().addPlayerStoneContrib(player, blueContribCount,
				...
				case 9: // Receive Contribution Rewards
					...
					if (SevenSigns.getInstance().isSealValidationPeriod() && playerCabal == winningCabal) {
						...
						player.sendPacket(
								SystemMessage2.obtainItems(SevenSigns.ANCIENT_ADENA_ID, ancientAdenaReward, 0));
						showChatWindow(player, 9, "a", false);
					}
					break;
				case 11: // Teleport to Hunting Grounds - deprecated, instead use
							// scripts_Util:QuestGatekeeper x y x 5575 price
					try {
						...
						if (ancientAdenaCost > 0)
							if (!player.getInventory().destroyItemByItemId(SevenSigns.ANCIENT_ADENA_ID,
									ancientAdenaCost)) {
								player.sendPacket(Msg.YOU_DO_NOT_HAVE_ENOUGH_ADENA);
								return;
							}
				...
				case 17: // Exchange Seal Stones for Ancient Adena (Type Choice) - SevenSigns 17 x
					...
					if (stoneType == 4) {
						...
						if (ancientAdenaReward > 0) {
							...
							player.sendMessage(new CustomMessage(
									"l2ft.gameserver.model.instances.L2SignsPriestInstance.DontHaveAnySS", player));
						break;
					}
					...
					if (content != null) {
						...
						_log.warn(
								"Problem with HTML text " + SevenSigns.SEVEN_SIGNS_HTML_PATH + "signs_17.htm: " + path);
					break;
				case 18: // Exchange Seal Stones for Ancient Adena - SevenSigns 18 xxxx xxxxxx
					...
					if (convertCount <= totalCount && convertCount > 0) {
						...
						if (player.getInventory().destroyItemByItemId(convertStoneId, convertCount)) {
							...
							ancientAdena.setCount(ancientAdenaReward);
							player.getInventory().addItem(ancientAdena);
							player.sendPacket(SystemMessage2.removeItems(convertStoneId, convertCount),
									SystemMessage2.obtainItems(SevenSigns.ANCIENT_ADENA_ID, ancientAdenaReward, 0));
						}
					} else
						player.sendMessage(new CustomMessage(
								"l2ft.gameserver.model.instances.L2SignsPriestInstance.DontHaveSSAmount", player));
					break;
				...
				case 21: // Exchange Adena for Ancient Adena - High Five
					...
					if (System.currentTimeMillis() > sh.getTimeInMillis()
							&& System.currentTimeMillis() < eh.getTimeInMillis())
				...
				case 22:
					...
					if (adenaAmount < adenaConvert || adenaConvert < tradeMult) // adenaConvert < tradeMult i.e. can't
																				// exchange if no AA will be given
					{
						player.sendPacket(Msg.YOU_DO_NOT_HAVE_ENOUGH_ADENA);
						return;
					}
					...
					if (ItemFunctions.removeItem(player, 57, adenaConvert, true) == adenaConvert) {
						...
						player.setVar("bmarketadena", player.getVarInt("bmarketadena") + result,
								reset.next(System.currentTimeMillis()));
	...
	public void showChatWindow(Player player, int val, Object... arg) {
		...
		switch (npcId) {
			...
			case 31126: // Blacksmith of Mammon
				// Remove the following block to allow any player to interact
				/*
				 * if (!player.isGM())
				 * switch (compWinner) {
				 * case SevenSigns.CABAL_DAWN:
...
// EOF C:\l2sq\Pac
// Project\Java\l2ft\gameserver\model\instances\SignsPriestInstance.java
