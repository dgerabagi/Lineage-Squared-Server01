...
/**
 * ĐšĐľĐ»Đ»ĐµĐşŃ†Đ¸ŃŹ ĐżŃ€ĐµĐ´ĐľĐżŃ€ĐµĐ´ĐµĐ»ĐµĐ˝Đ˝Ń‹Ń… ĐşĐ»Đ°Ń�Ń�ĐľĐ˛ Ń„Ń�Đ˝ĐşŃ†Đ¸Đą.
 *
 */
public class StatFunctions
{
	private static class FuncMultRegenResting extends Func
	{
		...
		{
			if(env.character.isPlayer() && env.character.getLevel() <= 40 && ((Player) env.character).getActiveClassClassId().getLevel() < 3 && stat == Stats.REGENERATE_HP_RATE)
					env.value *= 6.; // TODO: ĐżĐµŃ€ĐµĐ´ĐµĐ»Đ°Ń‚ŃŚ ĐşŃ€Đ°Ń�Đ¸Đ˛ĐµĐµ
			else
				env.value *= 1.5;
		}
	}
	...
	private static class FuncPDamageResists extends Func
	{
		...
		public void calc(Env env)
		{
			...
			// TODO ĐżĐµŃ€ĐµĐ´ĐµĐ»Đ°Ń‚ŃŚ Đ˝Đ° Ń‚Ń� Đ¶Đµ Ń�Đ¸Ń�Ń‚ĐµĐĽŃ�, Ń‡Ń‚Đľ Ń� ŃŤŃ„Ń„ĐµĐşŃ‚ĐľĐ˛
			WeaponTemplate weapon = env.character.getActiveWeaponItem();
			if(weapon == null)
	...
	private static class FuncAttributeAttackSet extends Func
	{
		...
		public void calc(Env env)
		{
			if(env.character.getPlayer().getActiveClassClassId().getType2() == ClassType2.Summoner)
