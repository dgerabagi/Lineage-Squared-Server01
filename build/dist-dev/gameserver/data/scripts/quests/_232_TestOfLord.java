...
public class _232_TestOfLord extends Quest implements ScriptFile
{
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		if(state == CREATED)
		{
			...
			if(event.equalsIgnoreCase("30565-12.htm") && st.getQuestItemsCount(IMMORTAL_FLAME) > 0)
			{
				...
				{
					st.addExpAndSp(434000, 60000); // FIXME: Ń†Đ¸Ń„Ń€Đ° ĐżŃ€Đ¸Đ±Đ»Đ¸Đ·Đ¸Ń‚ĐµĐ»ŃŚĐ˝Đ°ŃŹ
					st.giveItems(ADENA_ID, 100000); // FIXME: Ń� ĐżĐľŃ‚ĐľĐ»ĐşĐ°
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		if(_state == CREATED)
		{
			...
			if(st.getPlayer().getActiveClassClassId().getId() != 0x32)
			{
				st.exitCurrentQuest(true);
				return "30565-02.htm";
			}
