 ...
 * - bbs_homepage.htm (older style)
 * - bbs_partymatching.htm (new style)
 *
 * The code replicates the same placeholders and logic for both pages.
 */
public class HomePageCommunity extends Functions implements ScriptFile, ICommunityBoardHandler {
	...
	public String[] getBypassCommands() {
		// We handle 3 distinct commands: _bbslink, _bbsclass, _bbsPartymatching
	...
	public void onBypassCommand(Player player, String bypass) {
		...
		_log.info("HomePageCommunity onBypassCommand triggered: " + bypass);

		// e.g. _bbslink_class_sort_asc_page_charObjId_classPage
		StringTokenizer st = new StringTokenizer(bypass, "_");
		String mainToken = st.nextToken(); // "bbslink", "bbsclass", or "bbsPartymatching"

		_log.info("mainToken = " + mainToken);

		if ("bbslink".equals(mainToken)) {
			_log.info("Inside bbslink logic...");
			// The old logic referencing bbs_homepage.htm
			if (!st.hasMoreTokens()) {
				...
				_log.info("bbslink params: classesSortType=" + classesSortType +
						" sortType=" + sortType + " asc=" + asc + " page=" + page +
						" charObjId=" + charObjId + " classPage=" + classPage);
				...
				if (st.hasMoreTokens()) {
					...
					_log.info("Extra token in bbslink => nextNumber=" + nextNumber);
					...
					if (nextNumber == -1) {
						...
						_log.info("Visibility changed from " + oldState + " to " + player.isPartyMatchingVisible());
						// Refresh
						showMainPage(player, classesSortType, sortType, asc, page, charObjId, classPage);
					} else {
						// Invite to party with nextNumber as classId
						Player invited = GameObjectsStorage.getPlayer(charObjId);
						_log.info("Attempting to invite: " + invited + " for classId=" + nextNumber);
			...
			_log.info("Inside bbsclass logic...");
			// StackClass logic
			if (StackClassPage.canChangeClass(player)) {
				...
				if ("Primary".equals(type) || "Secondary".equals(type)) {
					...
					_log.info("nowSecondary=" + nowSecondary + " => finalClassId=" + choosenClassId);
				...
				_log.info("StackClassPage.canChangeClass returned false!");
		} else if ("bbsPartymatching".equals(mainToken)) {
			_log.info("Inside bbsPartymatching logic...");
			// new logic for bbs_partymatching.htm
			if (!st.hasMoreTokens()) {
				// no params => default
				...
				_log.info("bbsPartymatching params: classesSortType=" + classesSortType +
						" sortType=" + sortType + " asc=" + asc + " page=" + page +
						" charObjId=" + charObjId + " classPage=" + classPage);
				...
				if (st.hasMoreTokens()) {
					int nextNumber = Integer.parseInt(st.nextToken());
					_log.info("Extra token in bbsPartymatching => nextNumber=" + nextNumber);
			...
			_log.info("No match for mainToken=" + mainToken);
		}
	}
	...
	private void showMainPage(Player player, int classesSortType, int sortType,
		...
		_log.info("showMainPage called: cST=" + classesSortType + " sT=" + sortType + " asc=" + asc +
				" page=" + page + " charObjId=" + charObjId + " classPage=" + classPage);
		...
		html = html.replace("%classes%", getClasses(charObjId, classPage));
	...
	private void showPartyMatchingPage(Player player, int classesSortType, int sortType,
			int asc, int page, int charObjId, int classPage) {
		_log.info("showPartyMatchingPage called: cST=" + classesSortType + " sT=" + sortType + " asc=" + asc +
				" page=" + page + " charObjId=" + charObjId + " classPage=" + classPage);
	...
	// -------------------------------------------------------------------------
	// Common placeholders: %class%, %sort%, %asc%, etc.
	// -------------------------------------------------------------------------
	...
	// EXACTLY the same code from original "previous version"
	private String getCharacters(Player visitor, int charSort, int asc,
		...
		_log.info("getCharacters: charSort=" + charSort + " asc=" + asc +
				" classSort=" + classSort + " page=" + page + " charToView=" + charToView +
				" => total players in list=" + allPlayers.size());
		...
		for (int i = MAX_PER_PAGE * page; i < (MAX_PER_PAGE + badCharacters + page * MAX_PER_PAGE); i++) {
			...
			// "Classes" button
			html += "<td width=75><button value=\"Classes\" action=\"bypass _bbslink_%class%_%sort%_%asc%_%page%_"
					+ p.getObjectId()
					+ "_0\" width=80 height=18 back=\"L2UI_CT1.Button_DF_Down\" fore=\"L2UI_ct1.button_df\"></td>";
			html += "</tr></table>";
		}
	...
	private String getClasses(int charObjId, int page) {
		if (charObjId == 0)
			return "";
		...
		for (Object obj : playerToView.getUnlocks().getAllUnlocks().getValues()) {
			...
			// "Invite" button =>
			// _bbslink_%class%_%sort%_%asc%_%page%_%char%_%classPage%_cid
			html += "<td width=90><button value=\"Invite\" action=\"bypass _bbslink_%class%_%sort%_%asc%_%page%_%char%_%classPage%_"
					+ clazz.getId()
					+ "\" width=80 height=18 back=\"L2UI_CT1.Button_DF_Down\" fore=\"L2UI_ct1.button_df\"></td>";
			html += "</tr></table>";
			i++;
		}
		...
		if (page > 0) {
			html += "<td><center><button value=\"Prev\" action=\"bypass _bbslink_%class%_%sort%_%asc%_%page%_%char%_"
					+ (page - 1)
					+ "\" width=80 height=18 back=\"L2UI_CT1.Button_DF_Down\" fore=\"L2UI_ct1.button_df\"></center></td>";
		}
		if (nextPageOk) {
			html += "<td><center><button value=\"Next\" action=\"bypass _bbslink_%class%_%sort%_%asc%_%page%_%char%_"
					+ (page + 1)
					+ "\" width=80 height=18 back=\"L2UI_CT1.Button_DF_Down\" fore=\"L2UI_ct1.button_df\"></center></td>";
		}
		html += "</tr></table></center>";
	...
	private List<Player> getPlayerList(Player player, int sortType, int asc, int classSortType) {
		...
		if (classSortType == 8) // "Your Party" filter
		{
		...
		// Grab all players that can join
	...
	private ClassId[] getNeededClasses(int type) {
		switch (type) {
			...
			case 1: // Buffers
				return new ClassId[] {
						ClassId.hierophant, ClassId.evaSaint, ClassId.shillienSaint, ClassId.dominator,
						ClassId.doomcryer
				};
			case 2: // BD
			...
			case 5: // Tanks
				return new ClassId[] {
						ClassId.phoenixKnight, ClassId.hellKnight, ClassId.evaTemplar, ClassId.shillienTemplar
				};
			case 6: // Mage DD
				return new ClassId[] {
						ClassId.archmage, ClassId.soultaker, ClassId.arcanaLord, ClassId.mysticMuse,
						ClassId.elementalMaster, ClassId.stormScreamer, ClassId.spectralMaster,
						ClassId.dominator, ClassId.doomcryer
				};
			case 7: // Fighter DD
				return new ClassId[] {
						ClassId.dreadnought, ClassId.duelist, ClassId.adventurer, ClassId.sagittarius,
						ClassId.windRider, ClassId.moonlightSentinel, ClassId.ghostHunter, ClassId.ghostSentinel,
						ClassId.titan, ClassId.grandKhauatari, ClassId.fortuneSeeker
				};
			case 8: // "Your Party" => handled in getPlayerList
				return ClassId.values();
		}
		return ClassId.values();
	}
	...
	private static String getFullClassName(ClassId classIndex) {
		// same big switch from your code
		switch (classIndex) {
			case phoenixKnight:
