...
public class _412_PathToDarkwizard extends Quest implements ScriptFile
{
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		{
			if(st.getPlayer().getLevel() >= 18 && st.getPlayer().getActiveClassClassId().getId() == 0x26 && st.getQuestItemsCount(JEWEL_OF_DARKNESS_ID) < 1)
			{
				st.setCond(1);
				st.setState(STARTED);
			...
			else if(st.getPlayer().getActiveClassClassId().getId() != 0x26)
			{
				if(st.getPlayer().getActiveClassClassId().getId() == 0x27)
					htmltext = "varika_q0412_02a.htm";
				else
					htmltext = "varika_q0412_03.htm";
			}
			else if(st.getPlayer().getLevel() < 18 && st.getPlayer().getActiveClassClassId().getId() == 0x26)
				htmltext = "varika_q0412_02.htm";
			else if(st.getPlayer().getLevel() >= 18 && st.getPlayer().getActiveClassClassId().getId() == 0x26 && st.getQuestItemsCount(JEWEL_OF_DARKNESS_ID) > 0)
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == VARIKA)
		{
			...
			else if(st.getQuestItemsCount(SEEDS_OF_DESPAIR_ID) > 0 && st.getQuestItemsCount(SEEDS_OF_HORROR_ID) > 0 && st.getQuestItemsCount(SEEDS_OF_LUNACY_ID) > 0 && st.getQuestItemsCount(SEEDS_OF_ANGER_ID) > 0)
			{
				...
				if(st.getPlayer().getActiveClassClassId().getLevel() == 1)
				{
