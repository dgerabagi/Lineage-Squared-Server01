...
public class _211_TrialOfChallenger extends Quest implements ScriptFile
{
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		if(id == CREATED)
		{
			st.setCond(0);
			if(npcId == Kash)
				if(st.getPlayer().getActiveClassClassId().ordinal() == 0x01 || st.getPlayer().getActiveClassClassId().ordinal() == 0x13 || st.getPlayer().getActiveClassClassId().ordinal() == 0x20 || st.getPlayer().getActiveClassClassId().ordinal() == 0x2d || st.getPlayer().getActiveClassClassId().ordinal() == 0x2f)
