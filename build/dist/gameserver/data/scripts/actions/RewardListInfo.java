...
public abstract class RewardListInfo
{
	...
	public static void showInfo(Player player, NpcInstance npc)
	{
		...
		top += "</center><br><table><tr><td>Exp:"+npc.getExpReward()+"</td><td>Sp:"+npc.getSpReward()+"</td><td>Respawn:"+String.valueOf(npc.getSpawn() != null ? Util.formatTime(npc.getSpawn().getRespawnDelay()) : "0")+"</td></tr></table><center>";
		...
		for(Map.Entry<RewardType, RewardList> entry : npc.getTemplate().getRewards().entrySet())
		{
			...
			switch(entry.getKey())
			{
				...
				case NOT_RATED_NOT_GROUPED:
					...
					break;
				case SWEEP:
					notGroupedRewardList(builder, rewardList, Config.RATE_DROP_SPOIL * player.getRateSpoil(), mod, icons);
					break;
			}
		}
	...
	public static void ratedGroupedRewardList(StringBuilder tmp, NpcInstance npc, RewardList list, Player player, double mod, boolean icons)
	{
	...
	public static void notGroupedRewardList(StringBuilder tmp, RewardList list, double rate, double mod, boolean icons)
	{
		tmp.append("<table width=270 border=0>");
		tmp.append("<tr><td><img src=\"L2UI.SquareBlank\" width=270 height=10> </td></tr>");
		tmp.append("<tr><td><table width=270 border=0><tr><td><font color=\"aaccff\">").append(list.getType()).append("</font></td></tr></table></td></tr>");
