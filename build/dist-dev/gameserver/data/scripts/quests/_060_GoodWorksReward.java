...
public class _060_GoodWorksReward extends Quest implements ScriptFile
{
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		{
			if(profs.containsKey(st.getPlayer().getActiveClassClassId().getId()))
				return Mammon_dialog.sprintf(profs.get(st.getPlayer().getActiveClassClassId().getId()));
		}
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		if(_state == CREATED)
		{
			if(npcId != Daeger)
				return "noquest";
			if(st.getPlayer().getLevel() < 39 || st.getPlayer().getRace() == Race.kamael || st.getPlayer().getActiveClassClassId().level() != 1)
			{
