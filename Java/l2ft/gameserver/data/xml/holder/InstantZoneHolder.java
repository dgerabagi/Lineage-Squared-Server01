...
public class InstantZoneHolder extends AbstractHolder {
	...
	public List<Integer> getSharedReuseInstanceIds(int id) {
		...
		for (InstantZone iz : _zones.values())
			if (iz.getSharedReuseGroup() > 0 &&
					getInstantZone(id).getSharedReuseGroup() > 0 &&
	...
	/**
	 * Overridden to provide a detailed listing of all loaded InstantZones
	 */
	@Override
	public void log() {
		info("Loaded " + _zones.size() + " instant zone(s) total:");
		for (InstantZone iz : _zones.values()) {
			info("   ID=" + iz.getId() + "  Name=\"" + iz.getName() + "\"");
		}
	}
}
