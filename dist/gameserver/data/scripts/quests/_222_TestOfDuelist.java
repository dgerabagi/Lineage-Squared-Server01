...
/**
 * ĐšĐ˛ĐµŃ�Ń‚ Đ˝Đ° Đ˛Ń‚ĐľŃ€Ń�ŃŽ ĐżŃ€ĐľŃ„ĐµŃ�Ń�Đ¸ŃŽ Test Of Duelist
 */
public class _222_TestOfDuelist extends Quest implements ScriptFile
{
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == Kaien)
			...
			{
				if(st.getPlayer().getActiveClassClassId().getId() == 0x01 || st.getPlayer().getActiveClassClassId().getId() == 0x2f || st.getPlayer().getActiveClassClassId().getId() == 0x13 || st.getPlayer().getActiveClassClassId().getId() == 0x20)
				{
			...
			else if(cond == 5)
			{
				...
				if(!st.getPlayer().getVarB("prof2.3"))
				{
					...
					st.giveItems(ADENA_ID, 80000); // FIXME: Ń†Đ¸Ń„Ń€Đ° Ń� ĐżĐľŃ‚ĐľĐ»ĐşĐ°
