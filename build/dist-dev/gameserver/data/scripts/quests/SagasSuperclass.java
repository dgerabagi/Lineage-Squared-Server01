...
public abstract class SagasSuperclass extends Quest
{
	...
	private void FinishQuest(QuestState st, Player player)//TODO
	{
		st.addExpAndSp(2586527, 0);
		st.giveItems(ADENA_ID, 5000000);
		st.giveItems(6622, 1, true);
		st.exitCurrentQuest(true);
		//player.setClassId(getActiveClassClassId(player), false, true);
		//if(player.isOnMainClass() && player.getActiveClass().getFirstClassId() == getPrevClass(player))
			//player.setActiveClass(getActiveClassClassId(player));
	...
	protected int getActiveClassClassId(Player player)
	{
		return classid;
	}
	...
	public static QuestState findQuest(Player player)
	{
		...
		for(Integer q : Quests.keySet())
		{
			...
			if(st != null)
			{
				...
				for(int c : qc)
					if(player.getActiveClassClassId().getId() == c)
						return st;
			}
		}
		return null;
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		int cond = st.getCond();
		Player player = st.getPlayer();
		if(player.getActiveClassClassId().getId() != getPrevClass(player))
		{
			st.exitCurrentQuest(true);
			return htmltext;
		}
		...
		else if(cond == 19)
		{
			...
			if(npcId == NPC[0])
				...
				{
					htmltext = "0-09.htm";
					if(getActiveClassClassId(player) < 131 || getActiveClassClassId(player) > 135)
						FinishQuest(st, player);
				}
				else
					htmltext = "0-010.htm";
	...
	public String onKill(NpcInstance npc, QuestState st)
	{
		int npcId = npc.getNpcId();
		Player player = st.getPlayer();
		if(player.getFirstClassId() != getPrevClass(player))
			return null;
