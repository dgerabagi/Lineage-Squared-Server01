// C:\l2sq\Pac Project\Java\l2ft\gameserver\model\entity\SevenSigns.java
 ...
 *
 * TODO:
 * - Currently all those in the winning cabal can enter the catacombs and
 * necropolises.
 * The official site says this is seal-based rather than winner-based. If so,
 * then
...
public class SevenSigns {
	...
	public void spawnSevenSignsNPC() {
		AutoSpawnInstance _merchantSpawn = AutoSpawnManager.getInstance().getAutoSpawnInstance(MAMMON_MERCHANT_ID,
				false);
		AutoSpawnInstance _blacksmithSpawn = AutoSpawnManager.getInstance().getAutoSpawnInstance(MAMMON_BLACKSMITH_ID,
				false);
		Map<Integer, AutoSpawnInstance> _marketeerSpawns = AutoSpawnManager.getInstance()
				.getAllAutoSpawnInstance(MAMMON_MARKETEER_ID);
		...
		AutoSpawnInstance _crestOfDawnSpawn = AutoSpawnManager.getInstance().getAutoSpawnInstance(CREST_OF_DAWN_ID,
				false);
		AutoSpawnInstance _crestOfDuskSpawn = AutoSpawnManager.getInstance().getAutoSpawnInstance(CREST_OF_DUSK_ID,
				false);
		Map<Integer, AutoSpawnInstance> _oratorSpawns = AutoSpawnManager.getInstance()
				.getAllAutoSpawnInstance(ORATOR_NPC_ID);
		Map<Integer, AutoSpawnInstance> _preacherSpawns = AutoSpawnManager.getInstance()
				.getAllAutoSpawnInstance(PREACHER_NPC_ID);
	...
	public final long getCurrentScore(int cabal) {
		...
		switch (cabal) {
			case CABAL_NULL:
				return 0;
			case CABAL_DAWN:
				return Math.round(_dawnStoneScore / (totalStoneScore == 0 ? 1 : totalStoneScore) * 500)
						+ _dawnFestivalScore;
			case CABAL_DUSK:
				return Math.round(_duskStoneScore / (totalStoneScore == 0 ? 1 : totalStoneScore) * 500)
						+ _duskFestivalScore;
		}
		return 0;
	}
	...
	public long getPlayerAdenaCollect(Player player) {
		if (!hasRegisteredBefore(player.getObjectId()))
			return 0;
		return _signsPlayerData.get(player.getObjectId()).getLong(
				getPlayerCabal(player) == CABAL_DAWN ? "dawn_ancient_adena_amount" : "dusk_ancient_adena_amount");
	}
	 ...
	 * checks to see if the player has registered before and will update the
	 * database
	 * if necessary.
	 * <BR>
	...
	public int setPlayerInfo(int charObjId, int chosenCabal, int chosenSeal) {
		...
		switch (chosenCabal) {
			case CABAL_DAWN:
				contribScore = calcContributionScore(currPlayer.getInteger("dawn_blue_stones"),
						currPlayer.getInteger("dawn_green_stones"), currPlayer.getInteger("dawn_red_stones"));
				_dawnStoneScore += contribScore;
				break;
			case CABAL_DUSK:
				contribScore = calcContributionScore(currPlayer.getInteger("dusk_blue_stones"),
						currPlayer.getInteger("dusk_green_stones"), currPlayer.getInteger("dusk_red_stones"));
	...
	public long addPlayerStoneContrib(int charObjId, long blueCount, long greenCount, long redCount) {
		...
		if (currPlayer.getInteger("cabal") == CABAL_DAWN) {
			totalAncientAdena = currPlayer.getInteger("dawn_ancient_adena_amount")
			...
			totalAncientAdena = currPlayer.getInteger("dusk_ancient_adena_amount")
					+ calcAncientAdenaReward(blueCount, greenCount, redCount);
	...
	public void sendCurrentPeriodMsg(Player player) {
		switch (_activePeriod) {
			...
			case PERIOD_COMPETITION:
				player.sendPacket(
						Msg.SEVEN_SIGNS_THE_QUEST_EVENT_PERIOD_HAS_BEGUN_SPEAK_WITH_A_PRIEST_OF_DAWN_OR_DUSK_PRIESTESS_IF_YOU_WISH_TO_PARTICIPATE_IN_THE_EVENT);
				return;
			case PERIOD_COMP_RESULTS:
			...
			case PERIOD_SEAL_VALIDATION:
				player.sendPacket(
						Msg.SEVEN_SIGNS_THIS_IS_THE_SEAL_VALIDATION_PERIOD_A_NEW_QUEST_EVENT_PERIOD_BEGINS_NEXT_MONDAY);
				return;
		}
	}
	...
	/**
	 * Used to initialize the seals for each cabal. (Used at startup or at beginning
	 * of a new cycle).
	 * This method should be called after <B>resetSeals()</B> and
	 * <B>calcNewSealOwners()</B> on a new cycle.
	 */
	...
	protected void calcNewSealOwners() {
		for (Integer currSeal : _signsDawnSealTotals.keySet()) {
			...
			/*
			 * - If a Seal was already closed or owned by the opponent and the new winner
			 * wants
			 ...
			 *
			 * - If the Seal was owned by the winner in the previous Seven Signs, they can
			 * retain
			 * that seal if 10% or more members have chosen it. If they want to possess a
			 * new Seal,
			...
			switch (currSeal) {
				case SEAL_AVARICE:
					if (newSealOwner == CABAL_DAWN)
						...
						sendMessageToAll(
								SystemMessage.SEVEN_SIGNS_THE_REVOLUTIONARIES_OF_DUSK_HAVE_OBTAINED_THE_SEAL_OF_AVARICE);
					break;
				case SEAL_GNOSIS:
					if (newSealOwner == CABAL_DAWN)
						...
						sendMessageToAll(
								SystemMessage.SEVEN_SIGNS_THE_REVOLUTIONARIES_OF_DUSK_HAVE_OBTAINED_THE_SEAL_OF_GNOSIS);
					break;
				case SEAL_STRIFE:
					if (newSealOwner == CABAL_DAWN)
						...
						sendMessageToAll(
								SystemMessage.SEVEN_SIGNS_THE_REVOLUTIONARIES_OF_DUSK_HAVE_OBTAINED_THE_SEAL_OF_STRIFE);
					break;
			}
		}
	}
		...
		public void runImpl() throws Exception {
			...
			switch (periodEnded) {
				case PERIOD_COMP_RECRUITING: // Initialization
					sendMessageToAll(
							SystemMessage.SEVEN_SIGNS_THE_QUEST_EVENT_PERIOD_HAS_BEGUN_VISIT_A_PRIEST_OF_DAWN_OR_DUSK_TO_PARTICIPATE_IN_THE_EVENT);
					// раздаем награды за рейдов, к печатям не относится но чтобы не заводить еще
					// один таймер...
					RaidBossSpawnManager.getInstance().distributeRewards();
					break;
				case PERIOD_COMPETITION: // Results Calculation
					sendMessageToAll(
							SystemMessage.SEVEN_SIGNS_THE_QUEST_EVENT_PERIOD_HAS_ENDED_THE_NEXT_QUEST_EVENT_WILL_START_IN_ONE_WEEK);
			...
			try {
				...
				setCalendarForNextPeriodChange();
				_log.info("SevenSignsPeriodChange: time to next change="
						+ Util.formatTime((int) (getMilliToPeriodChange() / 1000L)));
	...
	public void setTimeToNextPeriodChange(int time) {
		...
		_periodChange = ThreadPoolManager.getInstance().schedule(new SevenSignsPeriodChange(),
...
// EOF C:\l2sq\Pac Project\Java\l2ft\gameserver\model\entity\SevenSigns.java
