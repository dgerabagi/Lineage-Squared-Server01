...
public class _405_PathToCleric extends Quest implements ScriptFile
{
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		if(event.equalsIgnoreCase("1"))
			if(st.getPlayer().getLevel() >= 18 && st.getPlayer().getActiveClassClassId().getId() == 0x0a && st.getQuestItemsCount(MARK_OF_FAITH) < 1)
			{
				st.setCond(1);
				st.setState(STARTED);
			...
			else if(st.getPlayer().getActiveClassClassId().getId() != 0x0a)
			{
				if(st.getPlayer().getActiveClassClassId().getId() == 0x0f)
					htmltext = "gigon_q0405_02a.htm";
				else
					htmltext = "gigon_q0405_02.htm";
			}
			else if(st.getPlayer().getLevel() < 18 && st.getPlayer().getActiveClassClassId().getId() == 0x0a)
				htmltext = "gigon_q0405_03.htm";
			else if(st.getPlayer().getLevel() >= 18 && st.getPlayer().getActiveClassClassId().getId() == 0x0a && st.getQuestItemsCount(MARK_OF_FAITH) > 0)
