...
public class Rename extends Functions
{
	...
	public void changebase_page()//TODO
	{
		/**Player player = getSelf();
		if(player == null)
			return;
		...
		List<ActiveClass> possible = new ArrayList<ActiveClass>();
		if(player.getActiveClass().isMain())
		{
			possible.addAll(player.getClasses().values());
			//possible.remove(player.getClasses().get(player.getBaseClassId()));
