...
public class RequestAquireSkill extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		{
			case NORMAL:
				learnSimpleNextLevel(player, skillLearn, skill, false, _type, false);
				if(trainer != null)
					trainer.showSkillList(player);
				break;
			case TRANSFORMATION:
				learnSimpleNextLevel(player, skillLearn, skill, true, _type, true);
				if(trainer != null)
			...
			case TRANSFER_SHILLIEN_SAINTS:
				...
				{
					learnSimple(player, skillLearn, skill, false, player.getFirstClassId());
					if(trainer != null)
						trainer.showTransferSkillList(player);
				}
				break;
			case FISHING:
				learnSimpleNextLevel(player, skillLearn, skill, true, _type, false);
				if(trainer != null)
	...
	/**
	 * Đ�Đ·Ń�Ń‡ĐµĐ˝Đ¸Đµ Ń�Đ»ĐµĐ´Ń�ŃŽŃ‰ĐµĐłĐľ Đ˛ĐľĐ·ĐĽĐľĐ¶Đ˝ĐľĐłĐľ Ń�Ń€ĐľĐ˛Đ˝ŃŹ Ń�ĐşĐ¸Đ»Đ»Đ°
	 */
	private static void learnSimpleNextLevel(Player player, SkillLearn skillLearn, Skill skill, boolean allClasses, AcquireType type, boolean justSingle)
	{
		final int skillLevel = player.getSkillLevel(skillLearn.getId(), 0);
		if(skillLevel != skillLearn.getLevel() - 1)
			return;
		int times = skillLearn.getLevel();
		if(player.getSkillLearnType() == SkillLearnType.All_At_Once && !justSingle)
			times = skill.getBaseLevel();
		int learnedLvl = skillLearn.getLevel();
		for(int i = skillLearn.getLevel();i<=times;i++)
		{
			skillLearn = SkillAcquireHolder.getInstance().giveSkillLearnWithoutQuestions(type, player, skillLearn.getId(), i);
			if(skillLearn == null)
				break;
			skill = SkillTable.getInstance().getInfo(skillLearn.getId(), i);
			if(!learnSimple(player, skillLearn, skill, allClasses, -1))
			{
				learnedLvl = skillLearn.getLevel()-1;
				break;
			}
		}
		player.sendUserInfo();
		...
		RequestExEnchantSkill.updateSkillShortcuts(player, skill.getId(), learnedLvl);
	}

	private static boolean learnSimple(Player player, SkillLearn skillLearn, Skill skill, boolean allClasses, int justClass)
	{
		...
		if(player.getLevel() < skillLearn.getMinLevel())
		{
			return false;
		}
		...
		if(allClasses)
			player.addSkill(skill, true, 0);
		else if(justClass > 0)
			player.addSkill(skill, true, justClass);
		else
	...
	private static void learnClanSkill(Player player, SkillLearn skillLearn, NpcInstance trainer, Skill skill)
	{
		...
		if(skillLevel != skillLearn.getLevel() - 1) // ĐĽĐľĐ¶Đ˝Đľ Đ˛Ń‹Ń�Ń‡Đ¸Ń‚ŃŚ Ń‚ĐľĐ»ŃŚĐşĐľ Ń�Đ»ĐµĐ´Ń�ŃŽŃ‰Đ¸Đą Ń�Ń€ĐľĐ˛ĐµĐ˝ŃŚ
			return;
	...
	private static boolean checkSpellbook(Player player, SkillLearn skillLearn)
	{
		...
		// Ń�ĐşĐ¸Đ»Ń‹ ĐżĐľ ĐşĐ»Đ¸ĐşŃ� Ń�Ń‡Đ°Ń‚Ń�ŃŹ Đ´Ń€Ń�ĐłĐ¸ĐĽ Ń�ĐżĐľŃ�ĐľĐ±ĐľĐĽ
