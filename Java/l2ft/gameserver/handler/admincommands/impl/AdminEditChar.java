...
public class AdminEditChar implements IAdminCommandHandler
{
	...
	public boolean useAdminCommand(Enum comm, String[] wordList, String fullString, Player activeChar)
	{
		...
		else if(fullString.equals("admin_show_reports"))
		{
			listReports(activeChar);
		}
		else if(fullString.startsWith("admin_clear_reports"))
		{
			String targetName = fullString.substring(20);
			Player player = World.getPlayer(targetName);
			BotReport.clearPlayerReports(player);
			listReports(activeChar);
		}
		...
		else if(fullString.startsWith("admin_find_dualbox"))
			findDualBoxes(activeChar);
		...
		{
			// ĐˇŃ‚Đ°Ń‚Ń�Ń� ĐĽĐµĐ˝ŃŹĐµŃ‚Ń�ŃŹ Ń‚ĐľĐ»ŃŚĐşĐľ Đ˝Đ° Ń‚ĐµĐşŃ�Ń‰Ń�ŃŽ Đ»ĐľĐłĐľĐ˝ Ń�ĐµŃ�Ń�Đ¸ŃŽ
			GameObject target = activeChar.getTarget();
			Player player;
		...
		{
			// ĐˇŃ‚Đ°Ń‚Ń�Ń� Ń�ĐľŃ…Ń€Đ°Đ˝ŃŹĐµŃ‚Ń�ŃŹ Đ˛ Đ±Đ°Đ·Đµ
			GameObject target = activeChar.getTarget();
			Player player;
			if(wordList.length > 1 && wordList[1] != null)
			{
	...
	private void listReports(Player activeChar)
	{
		Map<Player, Integer> reports = BotReport.getReports();
		...
		// List Reports in a Table
		replyMSG.append("<table width=270>");
		replyMSG.append("<tr><td width=80>Name:</td><td width=40>Reports:</td><td width=20>TP:</td><td width=30>Clear:</td></tr>");
		for(Entry<Player, Integer> report : reports.entrySet())
		{
			replyMSG.append("<tr><td width=80><font color="+getOnlineColor(report.getKey().isOnline())+">"+report.getKey().getName()+"</font></td><td width=40>" + report.getValue() + "</td>");
			replyMSG.append("<td width=20><button value=\"TP\" action=\"bypass -h admin_goto_char_menu "+report.getKey().getName()+"\" width=20 height=30 back=\"L2UI_CT1.Button_DF_Down\" fore=\"L2UI_ct1.button_df\"></td>");
			replyMSG.append("<td width=30><button value=\"Clear\" action=\"bypass -h admin_clear_reports "+report.getKey().getName()+"\" width=30 height=30 back=\"L2UI_CT1.Button_DF_Down\" fore=\"L2UI_ct1.button_df\"></td></tr>");
		}
	...
	private static String getOnlineColor(boolean isOnline)
	{
		return isOnline ? "4dc44d" : "c44d4d";
	}
	
	private static boolean _comparatorCausedError = false;
	
	private void listCharacters(Player activeChar, int page)
	{
		List<Player> players = GameObjectsStorage.getAllPlayers();
		
		if(!_comparatorCausedError)
		{
			try
			{
				Collections.sort(players, _comparator);
			}
			catch(Exception e)
			{
				_comparatorCausedError = true;
			}
		}
		
		int MaxCharactersPerPage = 50;
		...
		replyMSG.append("<table width=270>");
		replyMSG.append("<tr><td width=80>Name:</td><td width=40>Level:</td><td width=60>Port:</td><td width=50>State:</td></tr>");
		for(int i = CharactersStart; i < CharactersEnd; i++)
		{
			Player p = players.get(i);
			replyMSG.append("<tr><td width=80>" + "<a action=\"bypass -h admin_character_list " + p.getName() + "\">" + p.getName() + "</a></td>");
			replyMSG.append("<td width=40>" + p.getLevel() + "</td>");
			replyMSG.append("<td width=60><button value=\"Tele\" action=\"bypass -h admin_teleportto " + p.getName() + "\" width=60 height=20 back=\"L2UI_CT1.Button_DF_Down\" fore=\"L2UI_CT1.Button_DF\"></td>");
			...
			if(p.getKarma() > 0)
				...
				state = "PvP";
			else if(p.getPrivateStoreType() != 0)
				state = "Store";
			else if(p.isInCombatZone())
				state = "C Zone";
			else if(p.isInCombat())
				state = "Combat";
			else if(p.isInPeaceZone())
				state = "Peace";
			else
				state = "Normal";
			
			replyMSG.append("<td width=50>"+state+"</td></tr>");
		}
		replyMSG.append("</table>");
		replyMSG.append("</body></html>");
	...
	private CharacterComparator _comparator = new CharacterComparator();
	private class CharacterComparator implements Comparator<Player>
	{
		@Override
		public int compare(Player o1, Player o2)
		{
			if(o1.getKarma() > 0 && o2.getKarma() > 0)
				return 0;
			if(o1.getKarma() > 0)
				return -1;
			if(o2.getKarma() > 0)
			...
			if(o1.getPvpFlag() > 0 && o2.getPvpFlag() > 0)
				return 0;
			if(o1.getPvpFlag() > 0)
				return -1;
			if(o2.getPvpFlag() > 0)
			...
			if(o1.isInCombat() && o2.isInCombat())
				return 0;
			if(o1.isInCombat())
				return -1;
			if(o2.isInCombat())
			...
			if(!o1.isInPeaceZone() && !o2.isInPeaceZone())
				return 0;
			if(!o1.isInPeaceZone())
				return -1;
			if(!o2.isInPeaceZone())
			...
			if(o1.isInPeaceZone() && o1.getPrivateStoreType() == 0 && o2.isInPeaceZone() && o1.getPrivateStoreType() == 0)
				return 0;
			if(o1.isInPeaceZone() && o1.getPrivateStoreType() == 0)
				return -1;
			if(o2.isInPeaceZone() && o1.getPrivateStoreType() == 0)
				return 1;
			
			if(o1.getPrivateStoreType() > 0 && o2.getPrivateStoreType() > 0)
				return 0;
			if(o1.getPrivateStoreType() > 0)
				return -1;
			if(o2.getPrivateStoreType() > 0)
				return 1;
			return 0;
		}
	}
	
	private boolean isSamePc(Player player, Player box)
	{
		if(player == null || box == null)
		{
			return false;
		}
		if(player.getNetConnection() == null || box.getNetConnection() == null)
		{
			return false;
		}
		if(!player.getIP().equals(box.getIP()))
			return false;
		if(!player.getNetConnection().getRouterIpAddr().equals(box.getNetConnection().getRouterIpAddr()))
			return false;
		return true;
	}
	
	private void findDualBoxes(Player activeChar) {
		Map<Player, List<Player>> boxes = new HashMap<>();
		for(Player player : GameObjectsStorage.getAllPlayersForIterate())
		{
			boolean found = false;
			for(Player existingBoxClan : boxes.keySet())
				if(isSamePc(player, existingBoxClan))
				{
					boxes.get(existingBoxClan).add(player);
					found = true;
					break;
				}
			if(!found)
				for(Player box : GameObjectsStorage.getAllPlayersForIterate())
					if(player != box && isSamePc(player, box))
						{
							List<Player> pc = new ArrayList<>();
							pc.add(player);
							boxes.put(player, pc);
							break;
						}
		...
		replyMSG.append("</tr></table>");
		replyMSG.append("<br><center>Dual Boxers</center><br>");
		replyMSG.append("<table width=270>");
		for(List<Player> dualBoxList : boxes.values())
		{
			int boxesOutsideTown = 0;
			if(dualBoxList.size()>2)
				for(Player box : dualBoxList)
					if(!box.isInPeaceZone())
						boxesOutsideTown++;
			replyMSG.append("<tr>");
			for(Player dualBox : dualBoxList)
			{
				replyMSG.append("<td width=80>"+(boxesOutsideTown>2 ? "<font color=\"FF0000\">" : "")+"<a action=\"bypass -h admin_character_list " + dualBox.getName() + "\"> "+dualBox.getName()+" </a>"+(boxesOutsideTown > 2 ? "</font>" : "")+"</td>");
			}
			replyMSG.append("</tr>");
		}
		replyMSG.append("</table>");
	...
	public static void showCharacterList(Player activeChar, Player player)
	{
		...
		replyMSG.append("<tr><td width=100>Class/Id:</td><td>" + player.getTemplate().className + "/" + player.getActiveClass().getFirstClassId() + "</td></tr>");
	...
	private void editCharacter(Player activeChar)
	{
		...
		replyMSG.append("</table>");
		replyMSG.append("<table width=270><tr><td>Class:" + player.getActiveClass().getFirstClassId() + "</td></tr></table><br>");
		replyMSG.append("<table width=270>");
