...
public class AdminBan implements IAdminCommandHandler
{
				...
				case admin_jail:
					try
					{
						...
						while(st.hasMoreTokens())
							reason += st.nextToken()+" ";
						long realPeriod = Long.parseLong(period)*60*1000;
						Player target = World.getPlayer(player);

						if(target != null)
						{
							target.addNewTask(TaskType.Jail, realPeriod);
					...
					{
						activeChar.sendMessage("Command syntax: //jail char_name period_in_minutes reason");
					}
					break;
				case admin_unjail:
					try
					{
						...
						if(target != null && target.getVar("jailed") != null)
						{
							...
							target.removeWholeTask(TaskType.Jail);
							target.unblock();
							target.standUp();
	...
	private boolean tradeUnban(StringTokenizer st, Player activeChar)
	{
		...
		targ.removeWholeTask(TaskType.Trade_ban);
