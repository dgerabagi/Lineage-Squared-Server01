...
public abstract class RewardListInfo {
	...
	public static void showInfo(Player player, NpcInstance npc) {
		final int diff = npc
				.calculateLevelDiffForDrop(player.isInParty() ? player.getParty().getLevel() : player.getLevel());
		...
		String top = HtmlUtils.htmlNpcName(npc.getNpcId());
		top += "</center><br><table><tr><td>Exp:" + npc.getExpReward() + "</td><td>Sp:" + npc.getSpReward()
				+ "</td><td>Respawn:"
				+ String.valueOf(npc.getSpawn() != null ? Util.formatTime(npc.getSpawn().getRespawnDelay()) : "0")
				+ "</td></tr></table><center>";
		htmlMessage.replace("%npc_name%", top);
		...
		for (Map.Entry<RewardType, RewardList> entry : npc.getTemplate().getRewards().entrySet()) {
			...
			switch (entry.getKey()) {
				case RATED_GROUPED:
					ratedGroupedRewardList(builder, npc, rewardList, player, mod, icons);
					break;
				case NOT_RATED_GROUPED:
					notRatedGroupedRewardList(builder, rewardList, mod, icons);
					break;
				case NOT_RATED_NOT_GROUPED:
					notGroupedRewardList(builder, rewardList, 1.0, mod, icons);
					break;
				case SWEEP:
	...
	public static void ratedGroupedRewardList(StringBuilder tmp, NpcInstance npc, RewardList list, Player player,
			double mod, boolean icons) {
		...
		for (RewardGroup g : list) {
			...
			double rateDrop = (npc instanceof RaidBossInstance ? Config.RATE_DROP_RAIDBOSS
					: npc.isSiegeGuard() ? Config.RATE_DROP_SIEGE_GUARD
			...
			for (RewardData d : items) {
				...
				tmp.append("<tr><td width=32>");
				if (icons)
					tmp.append("<img src=").append(icon).append(" width=32 height=32>");
				tmp.append("</td><td width=238>").append(HtmlUtils.htmlItemName(d.getItemId())).append("<br1>");
				tmp.append("<font color=\"b09979\">[").append(Math.round(d.getMinDrop() * (g.isAdena() ? gmult : 1.0)))
						.append("..").append(Math.round(d.getMaxDrop() * imult)).append("]&nbsp;");
	...
	public static void notRatedGroupedRewardList(StringBuilder tmp, RewardList list, double mod, boolean icons) {
		tmp.append("<table width=270 border=0>");
		tmp.append("<tr><td><table width=270 border=0><tr><td><font color=\"aaccff\">").append(list.getType())
				.append("</font></td></tr></table></td></tr>");
		tmp.append("<tr><td><img src=\"L2UI.SquareWhite\" width=270 height=1> </td></tr>");
		...
		for (RewardGroup g : list) {
			...
			for (RewardData d : items) {
				...
				if (icons)
					tmp.append("<img src=").append(icon).append(" width=32 height=32>");
				tmp.append("</td><td width=238>").append(HtmlUtils.htmlItemName(d.getItemId())).append("<br1>");
				tmp.append("<font color=\"b09979\">[").append(Math.round(d.getMinDrop())).append("..")
						.append(Math.round(d.getMaxDrop())).append("]&nbsp;");
				tmp.append(pf.format(d.getChance() / RewardList.MAX_CHANCE)).append("</font></td></tr>");
			}
			tmp.append("</table></td></tr>");
		}
		...
		for (RewardGroup g : list) {
			...
			for (RewardData d : items) {
				...
				if (icons)
					tmp.append("<img src=").append(icon).append(" width=32 height=32>");
				tmp.append("</td><td width=238>").append(HtmlUtils.htmlItemName(d.getItemId())).append("<br1>");
				tmp.append("<font color=\"b09979\">[").append(d.getMinDrop()).append("..")
