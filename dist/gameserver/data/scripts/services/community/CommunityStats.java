...
public class CommunityStats implements ScriptFile, ICommunityBoardHandler
{
	...
	private void showCastle(Player player)
	{
		...
		{
			con = DatabaseFactory.getInstance().getConnection();
			statement = con.prepareStatement("SELECT clan_data.hasCastle,clan_data.clan_id FROM clan_data LEFT JOIN clan_subpledges ON clan_data.clan_id = clan_subpledges.clan_id  WHERE clan_data.clan_level>0 order by clan_data.clan_level desc limit 9;");
			rs = statement.executeQuery();
			StringBuilder html = new StringBuilder();
			html.append("<table width=570>");
			String Owner = null;
