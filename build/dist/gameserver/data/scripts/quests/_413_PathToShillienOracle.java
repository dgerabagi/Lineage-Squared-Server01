...
public class _413_PathToShillienOracle extends Quest implements ScriptFile
{
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		{
			if(st.getPlayer().getLevel() >= 18 && st.getPlayer().getActiveClassClassId().getId() == 0x26 && st.getQuestItemsCount(ORB_OF_ABYSS_ID) < 1)
				htmltext = "master_sidra_q0413_05.htm";
			else if(st.getPlayer().getActiveClassClassId().getId() != 0x26)
			{
				if(st.getPlayer().getActiveClassClassId().getId() == 0x2a)
			...
			else if(st.getPlayer().getLevel() >= 18 && st.getPlayer().getActiveClassClassId().getId() == 0x26 && st.getQuestItemsCount(ORB_OF_ABYSS_ID) > 0)
