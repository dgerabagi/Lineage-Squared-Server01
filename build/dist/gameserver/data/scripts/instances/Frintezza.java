...
public class Frintezza extends Reflection {
	...
	private static final int[] hallADoors = { 17130051, 17130052, 17130053, 17130054, 17130055, 17130056, 17130057,
			17130058 };
	private static final int[] corridorADoors = { 17130042, 17130043 };
	private static final int[] hallBDoors = { 17130061, 17130062, 17130063, 17130064, 17130065, 17130066, 17130067,
			17130068, 17130069, 17130070 };
	...
	private class Music extends RunnableImpl {
		...
		public void runImpl() throws Exception {
			...
			if (!frintezza.isBlocked()) {
				frintezza.broadcastPacket(
						new ExShowScreenMessage(song_name, 3000, ScreenMessageAlign.TOP_CENTER, true, 1, -1, true));
				frintezza.broadcastPacket(
				...
				// play)
				ThreadPoolManager.getInstance().schedule(new SongEffectLaunched(getSongTargets(song), song, 10000),
						10000);
			}
			// Schedule a new song to be played in 30-40 seconds...
			musicTask = ThreadPoolManager.getInstance().schedule(new Music(),
					_intervalOfFrintezzaSongs + Rnd.get(10000));
		}
		...
		private boolean minionsNeedHeal() {
			...
			if (weakScarlet != null && !weakScarlet.isAlikeDead()
					&& weakScarlet.getCurrentHp() < weakScarlet.getMaxHp() * 2 / 3)
				return true;
			if (strongScarlet != null && !strongScarlet.isAlikeDead()
					&& strongScarlet.getCurrentHp() < strongScarlet.getMaxHp() * 2 / 3)
				return true;
			for (int i = 0; i < 4; i++) {
				if (portraits[i] != null && !portraits[i].isDead()
						&& portraits[i].getCurrentHp() < portraits[i].getMaxHp() / 3)
					return true;
	...
	private class SongEffectLaunched extends RunnableImpl {
		...
		public void runImpl() throws Exception {
			...
			SongEffectLaunched songLaunched = new SongEffectLaunched(_targets, _song,
					_currentTime + _intervalOfFrintezzaSongs / 10);
	 ...
	 */
	private void showSocialActionMovie(NpcInstance target, int dist, int yaw, int pitch, int time, int duration,
			int socialAction) {
		if (target == null)
			return;
		for (Player pc : getPlayers())
	...
	private class SecondMorph extends RunnableImpl {
		...
		public void runImpl() throws Exception {
			try {
				switch (_taskId) {
					case 1:
						int angle = Math.abs((weakScarlet.getHeading() < 32768 ? 180 : 540)
	...
	private class ThirdMorph extends RunnableImpl {
		...
		public void runImpl() throws Exception {
			try {
				switch (_taskId) {
					case 1:
						_angle = Math.abs((weakScarlet.getHeading() < 32768 ? 180 : 540)
								- (int) (weakScarlet.getHeading() / 182.044444444));
						for (Player pc : getPlayers())
							pc.enterMovieMode();
						blockAll(true);
		...
		public void runImpl() throws Exception {
			try {
				switch (_taskId) {
					case 1:
						blockAll(true);
						int _angle = Math.abs((strongScarlet.getHeading() < 32768 ? 180 : 540)
				...
				if (self.getNpcId() == HallAlarmDevice) {
					...
					for (NpcInstance n : getNpcs())
						if (ArrayUtils.contains(blockANpcs, n.getNpcId()))
							n.getAI().notifyEvent(CtrlEvent.EVT_AGGRESSION,
									getPlayers().get(Rnd.get(getPlayers().size())), 200);
					...
					for (NpcInstance n : getNpcs())
						if ((ArrayUtils.contains(blockBNpcs, n.getNpcId())
								|| ArrayUtils.contains(blockANpcs, n.getNpcId())) && !n.isDead())
							return;
