...
public class _411_PathToAssassin extends Quest implements ScriptFile
{
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		{
			if(st.getPlayer().getLevel() >= 18 && st.getPlayer().getActiveClassClassId().getId() == 0x1f && st.getQuestItemsCount(IRON_HEART_ID) < 1)
			{
				st.setCond(1);
				st.setState(STARTED);
			...
			{
				if(st.getPlayer().getActiveClassClassId().getId() == 0x23)
			...
			else if(st.getPlayer().getLevel() >= 18 && st.getPlayer().getActiveClassClassId().getId() == 0x1f && st.getQuestItemsCount(IRON_HEART_ID) > 0)
